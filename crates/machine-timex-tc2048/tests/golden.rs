//! Boot golden screenshot for the Timex Computer 2048.
//!
//! Loads the TC2048 ROM, runs 200 frames, encodes the 704×296
//! hi-res-capable paletted framebuffer through `SPECTRUM_PALETTE`
//! as a 16-colour indexed PNG, and either writes it as a new
//! golden (when `UPDATE_GOLDENS=1` is set or the golden is
//! missing) or compares decoded bytes against the checked-in
//! golden in `tests/goldens/`.
//!
//! Same shape as the Sinclair-line `runtime-sinclair-zx-spectrum`
//! `goldens.rs` for boot screens, but using the Timex framebuffer
//! dimensions (`SCREEN_WIDTH_HIRES` × `SCREEN_HEIGHT`).
//!
//! `#[ignore]`d because not every developer has the ROM locally —
...
fn golden_tc2048_boot() {
    ...
    compare_or_update("tc2048-boot", &machine.framebuffer);
}
