//! `emu198x-oric-atmos` — Oric-1 / Atmos native binary.
//!
...
/// mode so existing invocations keep working; `--rom`/`--model` are shared with
/// the UI, so a bare `--model oric-1` opens the interactive window.
...
mod tests {
    ...
    #[test]
    fn detect_mode_treats_bare_model_as_ui() {
        let args = vec!["--model".to_owned(), "oric-1".to_owned()];
    ...
    fn detect_mode_recognises_script_via_automation_flags() {
        for flag in ["--script", "--frames", "--screenshot", "--audio-capture"] {
            let args = vec!["--rom".to_owned(), "oric.rom".to_owned(), flag.to_owned()];
            assert_eq!(
                detect_mode(&args),
                Mode::Script,
