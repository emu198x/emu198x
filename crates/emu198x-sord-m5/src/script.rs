...
ROM:
    --rom PATH                 Sord M5 BIOS ROM (monitor + BASIC-I)
                               default: $EMU198X_SORD_M5_ROM, then
...
/// # Errors
///
/// Returns an error for unreadable ROM/cart, script parse / exec
...
fn run_cli(cli: Cli) -> Result<serde_json::Value, String> {
    ...
    let cart = if let Some(cart_path) = cli.cart.as_deref() {
        Some(load_cart(cart_path)?)
    } else {
        None
    };
...
fn load_cart(path: &Path) -> Result<Vec<u8>, String> {
