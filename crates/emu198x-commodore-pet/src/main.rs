...
/// mode so existing invocations keep working; the ROM/`--columns` flags are
/// shared with the UI, so a bare `--columns 80` opens the interactive window.
...
mod tests {
    ...
    #[test]
    fn detect_mode_treats_bare_columns_as_ui() {
        let args = vec!["--columns".to_owned(), "80".to_owned()];
