...
//!
//! The ECS machine shared the OCS gap: no `$00E` arm in the custom-
//! register read dispatch, so CLXDAT fell through to the `_ => 0xFFFF`
//! open-bus default (every collision bit set, every read). The fix
//! routes `$00E` to ECS Super Denise's collision latch (via the shared
...
fn clxdat_read_is_wired_not_open_bus() {
    ...
    let amiga = AmigaEcs::new(vec![0u8; 512 * 1024]);
