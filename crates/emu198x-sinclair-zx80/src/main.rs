...
/// mode; `--rom` is shared with the UI, so a bare `--rom zx80.rom` opens the
/// interactive window.
...
mod tests {
    ...
    fn detect_mode_treats_bare_rom_as_ui() {
        let args = vec!["--rom".to_owned(), "zx80.rom".to_owned()];
    ...
    fn detect_mode_recognises_script_via_automation_flags() {
        for flag in ["--script", "--frames", "--screenshot", "--audio-capture"] {
            let args = vec!["--rom".to_owned(), "zx80.rom".to_owned(), flag.to_owned()];
            assert_eq!(
                detect_mode(&args),
                Mode::Script,
