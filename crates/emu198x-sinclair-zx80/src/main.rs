...
use machine_sinclair_zx80::{Zx80, FB_HEIGHT, FB_WIDTH};

fn usage() {
    eprintln!(
        "\
Usage: emu198x-sinclair-zx80 --rom PATH [OPTIONS]

Required:
    --rom PATH                4 KB ROM

Options:
    --ram-kb N                1 or 16 [default: 1]
...
fn run(cli: Cli) -> Result<(), String> {
    ...
    let mut sys = Zx80::new(rom, ram_size)?;
    for _ in 0..cli.frames {
        sys.run_frame();
    }
    println!(
        "ZX80 runtime: master_clock={} frames={} ram_kb={}",
        sys.master_clock(),
        sys.frame_count(),
        cli.ram_kb
    );
