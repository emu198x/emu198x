...
fn a_nop_costs_four_tstates() {
    ...
    let tstates_before = einstein.cpu_tstates();
    let retired_before = einstein.z80_instructions_retired();
    for _ in 0..4_000 {
        einstein.step_tick();
    }
    let tstates = einstein.cpu_tstates() - tstates_before;
    let retired = einstein.z80_instructions_retired() - retired_before;
