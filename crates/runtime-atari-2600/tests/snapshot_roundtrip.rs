//! Snapshot/restore round-trip coverage for the Atari 2600 runtime.
//!
...
fn run_to(runtime: &mut Atari2600Runtime, target: u64) {
    runtime
        .run_until(
...
fn snapshot_round_trip_preserves_live_state() {
    let mut runtime = Atari2600Runtime::blank(MODEL);
    run_to(&mut runtime, WARMUP);

    let snap = runtime.snapshot().expect("blank runtime should snapshot");

    let mut restored = Atari2600Runtime::blank(MODEL);
