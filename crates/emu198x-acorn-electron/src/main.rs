//! `emu198x-acorn-electron` — Electron native binary.
//!
...
/// mode so existing invocations keep working; `--os`/`--basic` are shared with
/// the UI, so a bare `--os os.rom` opens the interactive window.
...
mod tests {
    ...
    #[test]
    fn detect_mode_treats_bare_os_as_ui() {
        let args = vec!["--os".to_owned(), "os.rom".to_owned()];
    ...
    fn detect_mode_recognises_script_via_automation_flags() {
        for flag in ["--script", "--frames", "--screenshot", "--audio-capture"] {
            let args = vec!["--os".to_owned(), "os.rom".to_owned(), flag.to_owned()];
            assert_eq!(
                detect_mode(&args),
                Mode::Script,
