...
struct InlineTool {
    ...
    schema: Value,
    run: fn(Value, &mut AquariusSession) -> Result<Value, ToolError>,
}

impl Tool<AquariusSession> for InlineTool {
    fn name(&self) -> &str {
        self.name
    }
...
fn aq_ref(s: &AquariusSession) -> Result<&Aquarius, ToolError> {
    s.machine()
        .machine()
...
fn tool_query_cpu(_args: Value, session: &mut AquariusSession) -> Result<Value, ToolError> {
    let aq = aq_ref(session)?;
    let r = &aq.cpu().regs;
    Ok(json!({
        "a":  format!("${:02X}", r.a()),
        "af": format!("${:04X}", r.af),
        ...
        "im":   r.im,
        "halt": aq.cpu().halt,
        "tstates": aq.cpu_tstates(),
    }))
}

fn tool_memory_read(args: Value, session: &mut AquariusSession) -> Result<Value, ToolError> {
    ...
    for offset in 0..len {
        let byte = aq.peek(addr.wrapping_add(offset as u16));
        if offset > 0 {
            hex.push(' ');
        }
...
pub fn register_aquarius_tools(registry: &mut ToolRegistry<AquariusSession>) {
    fn add(
        registry: &mut ToolRegistry<AquariusSession>,
        name: &'static str,
        description: &'static str,
        schema: Value,
        run: fn(Value, &mut AquariusSession) -> Result<Value, ToolError>,
