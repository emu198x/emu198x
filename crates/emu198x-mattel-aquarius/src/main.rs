//! `emu198x-mattel-aquarius` — Aquarius native binary.
//!
...
/// mode so existing invocations keep working; `--bios`/`--char`/`--cart` are
/// shared with the UI, so a bare `--cart game.bin` opens the interactive window.
...
mod tests {
    ...
    fn detect_mode_treats_bare_cart_as_ui() {
        let args = vec!["--cart".to_owned(), "game.bin".to_owned()];
    ...
    fn detect_mode_recognises_script_via_automation_flags() {
        for flag in ["--script", "--frames", "--screenshot", "--audio-capture"] {
            let args = vec!["--cart".to_owned(), "game.bin".to_owned(), flag.to_owned()];
            assert_eq!(
                detect_mode(&args),
                Mode::Script,
