//! Minimal snapshot envelope for the Electron runtime.
...
struct ElectronRuntimeSnapshotV1 {
    version: u16,
    time: u64,
    model_id: String,
...
pub(crate) fn encode(runtime: &ElectronRuntime) -> Result<Vec<u8>, MachineError> {
    let snapshot = ElectronRuntimeSnapshotV1 {
...
pub(crate) fn decode(runtime: &mut ElectronRuntime, bytes: &[u8]) -> Result<(), MachineError> {
    ...
    runtime.set_rom_bytes(snapshot.os_bytes, snapshot.basic_bytes);
