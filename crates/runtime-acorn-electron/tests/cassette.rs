//! Cassette `load_media` wiring for the Acorn Electron (#394).
//!
//! Drives the runtime's UEF tape path end to end with synthetic 16 KiB ROMs, so
...
fn the_tape_survives_a_reset() {
    ...
    // A reset rebuilds the machine; the tape stays in the deck.
    assert!(rt.machine().expect("machine present").tape_loaded());
}
