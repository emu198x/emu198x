...
impl DeniseEcs {
    ...
    /// ECS Denise ID register value, as reported by DENISEID.
    /// High byte is $FF (open bus), low byte is $FC (Super Denise).
    /// Matches WinUAE: ecs_denise returns 0xFFFC.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn deniseid_matches_ecs_winuae_value() {
        let denise = DeniseEcs::new();
        assert_eq!(denise.deniseid(), 0xFFFC);
    }
