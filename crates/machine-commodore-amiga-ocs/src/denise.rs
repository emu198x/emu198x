...
impl Denise {
    ...
    pub fn tick(
        ...
        if in_viewport_h && in_viewport_v {
            ...
            //
            // Horizontal DIW values live in DIWSTRT/DIWSTOP low bytes and
            // are in lores-pixel units (= half-CCK). beam_x_lores combines
            // hpos (CCK) and phase (half-CCK). DIWSTRT has implicit H8=0
            // (range 0-255); DIWSTOP has implicit H8=1 (range 256-511).
            // No DENISE_H_OFFSET applies here — the WinUAE-aligned
            // BPLCON1 scroll comparator already accounts for the
            // Agnus→Denise pipeline delay one stage downstream.
