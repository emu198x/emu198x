...
fn run(amiga: &mut AmigaOcs, label: &str) {
    ...
    for _ in 0..(400 * PAL_FRAME_TICKS) {
        ...
        if pc == BEGIN_IO_ENTRY {
            entry += 1;
        } else if pc == BEGIN_IO_RTS_A {
            rts_a += 1;
        } else if pc == BEGIN_IO_RTS_B {
            rts_b += 1;
        }
        prev_pc = pc;
    }
    ...
    eprintln!(
        ...
        if (cia_a.external_a() & 0x04) != 0 {
            "1 (no change)"
        } else {
            "0 (change pending)"
        },
        if (cia_a.external_a() & 0x08) != 0 {
            "1 (not protected)"
        } else {
            "0 (protected)"
        },
        if (cia_a.external_a() & 0x10) != 0 {
            "1 (not at trk0)"
        } else {
            "0 (at trk0)"
        },
        if (cia_a.external_a() & 0x20) != 0 {
            "1 (NOT ready)"
        } else {
            "0 (ready)"
        },
    ...
    eprintln!(
        ...
        (cia_b.port_b_latch() >> 7) & 1,
        (cia_b.port_b_latch() >> 3) & 1,
        (cia_b.port_b_latch() >> 2) & 1,
        (cia_b.port_b_latch() >> 1) & 1,
        cia_b.port_b_latch() & 1,
    );
    eprintln!();
    ...
    eprintln!(
        "\n=== Disk-register write log ({} entries) ===",
