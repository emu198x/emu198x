...
fn dump(label: &str, use_slow_ram: bool) {
    ...
    eprintln!("\n########## {label} ##########");
    eprintln!("GfxBase+$32 (LOFlist) write log: {} entries", amiga.debug_watch_writes.len());
    for (cck, pc, addr, val, is_word) in amiga.debug_watch_writes.iter() {
        let frame = cck / 70824;
        let size = if *is_word { 'W' } else { 'B' };
        eprintln!(
            "  frame~{frame:<3}  pc=${pc:08X}  {size} write to ${addr:08X} = ${val:04X}"
        );
    }
}
...
#[test]
#[ignore]
fn slow_ram_loflist_writes() { dump("slow-RAM", true); }

#[test]
#[ignore]
fn chip_only_loflist_writes() { dump("chip-only", false); }
