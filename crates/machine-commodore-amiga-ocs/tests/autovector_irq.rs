...
fn build_test_rom() -> Vec<u8> {
    ...
    put_w(&mut rom, at, 0x23FC);
    at += 2;
    put_l(&mut rom, at, 0x00FC_0200);
    at += 4;
    put_l(&mut rom, at, 0x0000_006C);
    at += 4;

    // 3. Lower SR interrupt mask so IPL ≥ 1 will fire.
    //    ANDI.W #$F8FF, SR
    //      $027C F8FF
    put_w(&mut rom, at, 0x027C);
    at += 2;
    put_w(&mut rom, at, 0xF8FF);
    at += 2;
    ...
    at += 2;
    put_w(&mut rom, at, 0xC020);
    at += 2;
    put_l(&mut rom, at, 0x00DF_F09A);
    at += 4;
    ...
    put_w(&mut rom, at, 0x33FC);
    at += 2;
    put_w(&mut rom, at, 0x8020);
    at += 2;
    put_l(&mut rom, at, 0x00DF_F09C);
    at += 4;
