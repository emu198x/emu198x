...
fn a_nop_costs_four_tstates() {
    // 16 KB of `NOP`: the 8 KB OS ROM plus one 8 KB paged ROM, the minimum
    // `Mtx::new` accepts.
    let mut mtx = Mtx::new(vec![0x00u8; 0x4000], MtxModel::Mtx512).expect("build machine");
    ...
    let tstates_before = mtx.master_clock();
    let retired_before = mtx.z80_instructions_retired();
    for _ in 0..4_000 {
        mtx.step_tick();
    }
    let tstates = mtx.master_clock() - tstates_before;
    let retired = mtx.z80_instructions_retired() - retired_before;
