//! Tom Harte-style single-step harness for the 68040.
//!
//! There is no upstream Tom Harte corpus for the 68040 today, so the
...
//! of the 68000 fixture format — the 68040-only registers (`msp`,
//! `vbr`, `cacr`, `caar`) live in extra fields that are zero on 68000
//! fixtures.
//!
//! Default corpus root:
//!   `~/Projects/198x/assets/test-suites/m68k-generated/m68040/v1/`
//!
//! Override with the `M68040_TEST_DATA` environment variable.
//!
//! Each fixture covers one instruction (or one addressing-mode variant
//! of an instruction). The current `Cpu68040` type is still aliased to
//! `Cpu68000` — this harness is the baseline measurement that
//! `knowledge/decisions/motorola-68040-implementation-plan.md` Phase 0
//! calls for: the pass rate before any 68040-specific work begins.
//!
//! Skipped under normal `cargo test`. Run explicitly with:
//!
//! ```sh
//! cargo test -p motorola-68040 --test tom_harte -- --ignored --nocapture
//! ```
...
struct CpuState {
    ...
    prefetch: [u16; 2],
    ram: Vec<(u32, u8)>,
    // 68040+ supersets — absent on 68000 files, default to zero.
    #[serde(default)]
    msp: u32,
    #[serde(default)]
    vbr: u32,
...
fn candidate_fixture_roots() -> Vec<PathBuf> {
    ...
    if let Ok(path) = std::env::var("M68040_TEST_DATA") {
        roots.push(PathBuf::from(path));
    }
    roots.push(home.join("Projects/198x/assets/test-suites/m68k-generated/m68040/v1"));
    roots
}
...
fn fixture_root() -> PathBuf {
    candidate_fixture_roots()
        ...
        .unwrap_or_else(|| {
            let home = std::env::var("HOME").expect("HOME set");
            PathBuf::from(home).join("Projects/198x/assets/test-suites/m68k-generated/m68040/v1")
        })
}
...
fn apply_initial(cpu: &mut Cpu68040, mem: &mut SparseMem, initial: &CpuState) {
    ...
    // 68040-only register state (msp / vbr / cacr / caar) is ignored
    // for now — the current Cpu68040 is a type alias of Cpu68000 and
    // has no fields to receive these. They'll start mattering once
    // Phase 1 forks Cpu68040 into its own struct.
...
fn service_bus(cpu: &mut Cpu68040, mem: &mut SparseMem) {
...
fn run_one_instruction(cpu: &mut Cpu68040, mem: &mut SparseMem) -> bool {
...
fn compare_final(cpu: &Cpu68040, mem: &SparseMem, final_state: &CpuState) -> Vec<Mismatch> {
    let mut v = Vec::new();
...
/// `Cpu68040` is still aliased to `Cpu68000`, so anything that
/// requires 68040 semantics (bitfield ops, scaled index, MULL/DIVL,
...
fn harte_baseline_full_sweep() {
    ...
    println!();
    println!(
        "Tom Harte 68040 baseline ({} fixtures, Cpu68040 = Cpu68000):",
        entries.len()
    );
...
/// the corpus and should pass on the type-alias 68040 today. If this
...
fn harte_nop_smoke() {
    ...
    assert_eq!(
        r.passed, r.total,
        "NOP should pass on Cpu68000-aliased Cpu68040 — harness wiring is broken if it doesn't",
    );
}
