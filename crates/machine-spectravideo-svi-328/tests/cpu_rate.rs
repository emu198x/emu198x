...
fn a_nop_costs_four_tstates() {
    ...
    let mut svi = Svi328::new(vec![0x00u8; 32 * 1024], SviRegion::Pal);
    ...
    let tstates_before = svi.cpu_tstates();
    let retired_before = svi.z80_instructions_retired();
    for _ in 0..4_000 {
        svi.step_tick();
    }
    let tstates = svi.cpu_tstates() - tstates_before;
    let retired = svi.z80_instructions_retired() - retired_before;
