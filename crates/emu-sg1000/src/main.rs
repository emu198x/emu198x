...
//!
//! Runs the SG-1000 with a winit window and wgpu renderer, or in
...
fn print_usage() {
    eprintln!("Usage: emu-sg1000 [OPTIONS]");
    eprintln!();
    eprintln!("Options:");
    eprintln!("  --rom <file>         SG-1000 cartridge ROM file");
...
impl App {
    fn new(system: Sg1000, menu: Menu, menu_ids: MenuIds, region: Sg1000Region) -> Self {
        let frame_duration = match region {
            Sg1000Region::Ntsc => FRAME_DURATION_NTSC,
            Sg1000Region::Pal => FRAME_DURATION_PAL,
        };
        Self {
            system,
            window: None,
