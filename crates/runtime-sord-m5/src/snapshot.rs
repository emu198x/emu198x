//! Postcard-encoded snapshot envelope for the Sord M5 runtime.
//!
//! Serialises the **live machine** (Z80, TMS9918 VDP, SN76489 PSG, Z80 CTC,
//! cart ROM/RAM, and RAM) so a restore resumes exactly, rather than the old
//! bootstrap envelope that cold-booted from the ROM. Mirrors the SG-1000 /
//! Game Boy shape: a borrowing envelope for encode (no clone), an owning
//! envelope for decode.
...
#[derive(Serialize)]
struct M5RuntimeSnapshotRefV2<'a> {
    version: u16,
    time: u64,
    model_id: &'a str,
    machine: Option<&'a SordM5>,
}

/// Owning envelope used during decode.
#[derive(Deserialize)]
struct M5RuntimeSnapshotV2 {
    version: u16,
    time: u64,
    model_id: String,
...
pub(crate) fn encode(runtime: &M5Runtime) -> Result<Vec<u8>, MachineError> {
    let snapshot = M5RuntimeSnapshotRefV2 {
...
pub(crate) fn decode(runtime: &mut M5Runtime, bytes: &[u8]) -> Result<(), MachineError> {
    let snapshot: M5RuntimeSnapshotV2 =
...
#[cfg(test)]
mod tests {
    use super::{M5RuntimeSnapshotRefV2, decode};
    ...
    fn decode_rejects_unsupported_version() {
        let mut runtime = M5Runtime::blank(Model::M5Ntsc);
        let bytes = postcard::to_allocvec(&M5RuntimeSnapshotRefV2 {
            version: 999,
            time: 0,
            model_id: runtime.model().model_id(),
            machine: None,
