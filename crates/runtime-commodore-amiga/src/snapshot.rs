...
// Version 27 rejects version-26 snapshots because they cannot preserve
// Denise's prior-line raster carry and HBLANK-reset context. Version 26 had
// already crossed the per-line BPL1DAT sprite-visibility boundary, so older
// positional payloads also remain incompatible.
...
struct SnapshotEnvelopeV27<M: AmigaMachine> {
    version: u32,
    config: AmigaConfig,
    time: MachineTime,
    machine: M::Snapshot,
...
pub(crate) fn encode<M: AmigaMachine>(runtime: &AmigaRuntime<M>) -> Result<Vec<u8>, MachineError> {
    let envelope = SnapshotEnvelopeV27::<M> {
...
mod tests {
    ...
    fn restore_rejects_out_of_range_audio_phase_without_mutating_runtime() {
        ...
        let mut envelope: SnapshotEnvelopeV27<AmigaOcs> =
