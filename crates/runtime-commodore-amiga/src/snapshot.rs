...
// Version 28 rejects version-27 snapshots because they cannot preserve ECS
// Agnus's BLANKEN-qualified horizontal-blank event history or AGA Lisa's
// selector-qualified horizontal-blank latch. Older positional payloads also
// remain incompatible.
...
struct SnapshotEnvelopeV28<M: AmigaMachine> {
    version: u32,
    config: AmigaConfig,
    time: MachineTime,
    machine: M::Snapshot,
...
pub(crate) fn encode<M: AmigaMachine>(runtime: &AmigaRuntime<M>) -> Result<Vec<u8>, MachineError> {
    let envelope = SnapshotEnvelopeV28::<M> {
...
mod tests {
    ...
    fn restore_rejects_out_of_range_audio_phase_without_mutating_runtime() {
        ...
        let mut envelope: SnapshotEnvelopeV28<AmigaOcs> =
