...
// Version 29 rejects version-28 snapshots because they cannot preserve the
// keyboard byte awaiting a host handshake or the A600's model-specific Gayle
// state. Older positional payloads also remain incompatible.
...
struct SnapshotEnvelopeV29<M: AmigaMachine> {
    version: u32,
    config: AmigaConfig,
    time: MachineTime,
    machine: M::Snapshot,
...
pub(crate) fn encode<M: AmigaMachine>(runtime: &AmigaRuntime<M>) -> Result<Vec<u8>, MachineError> {
    let envelope = SnapshotEnvelopeV29::<M> {
...
mod tests {
    ...
    fn restore_rejects_out_of_range_audio_phase_without_mutating_runtime() {
        ...
        let mut envelope: SnapshotEnvelopeV29<AmigaOcs> =
