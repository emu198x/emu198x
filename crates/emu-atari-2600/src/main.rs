//! Atari 2600 emulator binary.
//!
//! Runs the 2600 with a winit window and wgpu renderer, or in
...
use emu_atari_2600::{
    Atari2600, Atari2600Config, Atari2600Region, capture,
    controller_map::{self, Atari2600Input},
    tia,
};
...
const FB_WIDTH: u32 = tia::FB_WIDTH;
...
fn print_usage() {
    eprintln!("Usage: emu-atari-2600 [OPTIONS]");
    eprintln!();
    eprintln!("Options:");
    eprintln!("  --rom <file>         Atari 2600 ROM file (.bin/.a26)");
        ...
        match args[i].as_str() {
            ...
            "--region" => {
                ...
                cli.region = match value.to_lowercase().as_str() {
                    "ntsc" => Atari2600Region::Ntsc,
                    "pal" => Atari2600Region::Pal,
...
struct App {
    ...
    menu_ids: MenuIds,
    _menu: Menu,
    /// RIOT port A state (joystick, active low — all released = 0xFF).
    swcha: u8,
    /// RIOT port B state (switches, active low — all released = 0xFF).
    swchb: u8,
}

impl App {
    fn new(system: Atari2600, menu: Menu, menu_ids: MenuIds) -> Self {
        let frame_duration = match system.region() {
            Atari2600Region::Ntsc => FRAME_DURATION_NTSC,
            Atari2600Region::Pal => FRAME_DURATION_PAL,
        };
        Self {
            system,
            window: None,
    ...
    fn handle_key(&mut self, keycode: KeyCode, pressed: bool) {
        if let Some(input) = controller_map::map_keycode(keycode) {
            if input == Atari2600Input::P0Fire {
                self.system.set_fire_button_p0(pressed);
            } else {
                // Joystick directions → SWCHA
                if let Some(bit) = controller_map::swcha_bit(input) {
                    if pressed {
                        self.swcha &= !bit; // Active low: clear bit
                    } else {
                        self.swcha |= bit; // Released: set bit
                    }
                    self.system.set_joystick_input(self.swcha);
                }
                // Console switches → SWCHB
                if let Some(bit) = controller_map::swchb_bit(input) {
                    if pressed {
                        self.swchb &= !bit;
                    } else {
                        self.swchb |= bit;
                    }
                    self.system.set_switch_input(self.swchb);
                }
            }
        }
    }
...
fn make_system_result(cli: &CliArgs) -> Result<Atari2600, String> {
    let rom_path = cli
        .rom_path
        .as_ref()
    ...
    Atari2600::new(&config)
}

fn make_system(cli: &CliArgs) -> Atari2600 {
...
mod tests {
    ...
    fn cli_parser_reads_basic_options() {
        ...
        assert_eq!(cli.rom_path, Some(PathBuf::from("combat.bin")));
        assert!(cli.headless);
        assert_eq!(cli.region, Atari2600Region::Pal);
    ...
    fn cli_parser_rejects_unknown_args() {
        let result = parse_cli(&["emu-atari-2600", "--bogus"]);
    ...
    fn cli_parser_help_returns_none() {
        assert!(matches!(
            parse_cli(&["emu-atari-2600", "--help"]).expect("help parse should succeed"),
            None
        ));
    }
}
