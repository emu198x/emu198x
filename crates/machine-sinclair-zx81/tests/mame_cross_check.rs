//! Cross-check the ZX81 boot screen against MAME.
//!
...
//! ZX81's picture at x=54, 26 pixels left of the ZX80's x=80. Retaining all
...
//! EMU198X_ZX81_MAME_PNG=/tmp/zx8x/zx81-boot-mame.png cargo test -p machine-sinclair-zx81 --test mame_cross_check
//! ```
//!
...
fn the_text_area_matches_mame() {
    let Ok(mame_path) = env::var("EMU198X_ZX81_MAME_PNG") else {
        emu198x_test_skip::skip!(
            "no MAME capture — run tools/zx8x-mame-capture/capture.sh and set EMU198X_ZX81_MAME_PNG"
        );
    };
    ...
    let golden_path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/goldens/zx81-boot.png");
