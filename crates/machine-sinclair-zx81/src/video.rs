...
///
/// The ZX81 has no frame counter. The picture is generated by the ROM, so the
/// frame length is whatever the ROM makes it, and a host budget must sit **at
/// or below the shortest frame the machine will emit** -- one longer leaves
/// the clock short of its target after a frame and the host runs a second,
/// which is the machine at double speed.
///
/// `TSTATES_PER_FRAME` (`207 x 312` = 64,584) is the wrong figure for this. It
/// is the *backstop*, the length a field runs to when no software sync ends it
/// first, so it is the **longest** frame rather than the shortest. Measured
/// against the stock ROM, a 50 Hz machine emits four lengths: 64,583 and
/// 64,584 while the ROM is still starting up and has not established its
/// sync, then 64,163 and 64,194 once it settles. Budgeting the backstop
/// therefore ran two frames per requested frame in the state the machine
/// spends all its time in.
///
/// 64,163 is that settled minimum, so every frame is at least this long and
/// exactly one runs per request.
///
/// ⚠ This is the stock ROM in SLOW mode at 50 Hz. It is not a floor for
/// arbitrary software: [`MIN_FRAME_T`] is the only real guarantee, and a
/// program driving its own sync near that would still double-step. A host
/// budget cannot express a frame length that varies per program, which is a
/// limitation of the pacing contract rather than of this constant.
pub const SLOW_MODE_FRAME_TSTATES: u32 = 64_163;
