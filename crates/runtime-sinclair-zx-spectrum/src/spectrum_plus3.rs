//! +3-specific runtime extras: firmware constructors and blank-runtime
//! helpers. Same 4-ROM Amstrad gate-array layout as the +2A and +2B,
//! sharing the `sinclair-zx-spectrum-plus3-rom-{0..3}` firmware ids.
//! ROM 2 carries the +3 DOS payload (only the +3 has a built-in
//! 3" floppy drive); the µPD765A FDC and disk slot are gated on the
//! `Plus3Marker::HAS_FDC` const at the type level.
...
impl SpectrumRuntime<SpectrumPlus3> {
    /// Builds a +3 runtime around the supplied 4-ROM image set.
    #[must_use]
    pub fn new_plus3(roms: [[u8; ROM_BYTES]; 4]) -> Self {
        let mut machine = SpectrumPlus3::new();
        machine
            .memory
            .load_roms(&roms[0], &roms[1], &roms[2], &roms[3]);
        SpectrumRuntime::new(Model::SpectrumPlus3, machine)
    }

    /// Builds a +3 runtime from borrowed 16 KiB ROM byte slices.
    ///
    /// # Errors
    ///
    ...
    pub fn from_rom_bytes(
        ...
        Ok(Self::new_plus3(roms))
    }

    /// Builds a +3 runtime from the profile-declared firmware set.
    ///
    /// # Errors
    ///
    ...
    pub fn from_firmware(firmware: &FirmwareSet<'_>) -> Result<Self, MachineError> {
        let profile = profile_for(Model::SpectrumPlus3);
...
mod tests {
    ...
    fn from_firmware_reports_invalid_rom_when_size_mismatches() {
        ...
        match SpectrumRuntime::<SpectrumPlus3>::from_firmware(&firmware) {
            ...
            Ok(_) => panic!("wrong-size +3 ROM 0 must be rejected"),
        }
    }
    ...
    fn from_rom_bytes_rejects_wrong_size_slice() {
        let full = [0u8; ROM_BYTES];
        match SpectrumRuntime::<SpectrumPlus3>::from_rom_bytes(&[0u8; 1024], &full, &full, &full) {
    ...
    #[test]
    fn blank_runtime_carries_the_plus3_profile_id() {
        use emu198x_shell::MachineCore;
        let runtime = SpectrumPlus3Runtime::blank();
        assert_eq!(
            runtime.profile().profile_id.as_str(),
            "sinclair-zx-spectrum-plus3-pal"
        );
    }
}
