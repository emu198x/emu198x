...
#[test]
#[ignore = "FIXTURE: needs 48K ROM and the Head over Heels SpeedLock-2 TZX"]
fn head_over_heels_speedlock2_reaches_game_code() {
    let firmware_root = home().join(".emu198x/roms/sinclair-zx-spectrum-48k");
    let tzx_path = spectrum_tzx_root().join(
        "ARCADE COLLECTION 12 - Head over Heels (1990)(Hit Squad, The)(48K-128K)[SpeedLock 2].zip",
    );
    if !firmware_root.exists() || !tzx_path.exists() {
        emu198x_test_skip::skip!("Spectrum 48K ROM or SpeedLock-2 TZX not staged");
    }
    ...
    let tape = read_media_asset(&tzx_path, MediaKind::Tape).expect("SpeedLock-2 TZX");
    let mut media = MediaSet::new();
    media.push(MediaImage::new(
        "tape-1".to_owned(),
        MediaKind::Tape,
    ...
    session
        .run_frames(16_000)
        .expect("run through protected load");
    let machine = session.machine().machine();
    let pc = machine.z80().regs.pc;
    assert_eq!(
        machine.tape().span_position().0,
        machine.tape().span_position().1
    );
    assert!(
        (0x7000..0x8000).contains(&pc),
        "SpeedLock-2 did not reach Head over Heels game code: PC=${pc:04x}"
    );
}
