//! Atari 5200 emulator binary.
//!
//! Runs the 5200 with a winit window and wgpu renderer, or in
...
use emu_atari_5200::{
    Atari5200, Atari5200Config, Atari5200Region, capture,
    controller_map::{self, Atari5200Input, POT_CENTER, POT_MAX, POT_MIN},
    gtia,
};
...
fn print_usage() {
    eprintln!("Usage: emu-atari-5200 [OPTIONS]");
    eprintln!();
    eprintln!("Options:");
    eprintln!("  --rom <file>         Atari 5200 cartridge ROM file");
    eprintln!("  --bios <file>        Optional 2KB BIOS ROM");
        ...
        match args[i].as_str() {
            ...
            "--region" => {
                ...
                cli.region = match value.to_lowercase().as_str() {
                    "ntsc" => Atari5200Region::Ntsc,
                    "pal" => Atari5200Region::Pal,
...
struct App {
    ...
    menu_ids: MenuIds,
    _menu: Menu,
    /// Joystick X axis (0=left, 114=center, 228=right).
    joy_x: u8,
    /// Joystick Y axis (0=up, 114=center, 228=down).
    joy_y: u8,
}

impl App {
    fn new(system: Atari5200, menu: Menu, menu_ids: MenuIds) -> Self {
        let frame_duration = match system.region() {
            Atari5200Region::Ntsc => FRAME_DURATION_NTSC,
            Atari5200Region::Pal => FRAME_DURATION_PAL,
        };
        Self {
            system,
            window: None,
    ...
    fn handle_key(&mut self, keycode: KeyCode, pressed: bool) {
        if let Some(input) = controller_map::map_keycode(keycode) {
            match input {
                Atari5200Input::P0Fire => {
                    self.system.set_fire(pressed);
                }
                Atari5200Input::P0Up => {
                    self.joy_y = if pressed { POT_MIN } else { POT_CENTER };
                    self.system.set_joystick(self.joy_x, self.joy_y);
                }
                Atari5200Input::P0Down => {
                    self.joy_y = if pressed { POT_MAX } else { POT_CENTER };
                    self.system.set_joystick(self.joy_x, self.joy_y);
                }
                Atari5200Input::P0Left => {
                    self.joy_x = if pressed { POT_MIN } else { POT_CENTER };
                    self.system.set_joystick(self.joy_x, self.joy_y);
                }
                Atari5200Input::P0Right => {
                    self.joy_x = if pressed { POT_MAX } else { POT_CENTER };
                    self.system.set_joystick(self.joy_x, self.joy_y);
                }
                Atari5200Input::Start => {
                    self.system.set_start(pressed);
                }
                Atari5200Input::Pause | Atari5200Input::Reset => {
                    // Pause/reset not yet wired
                }
            }
        }
    }
...
fn make_system_result(cli: &CliArgs) -> Result<Atari5200, String> {
    let rom_path = cli
        .rom_path
        .as_ref()
    ...
    let bios_data = if let Some(ref bios_path) = cli.bios_path {
        let data = std::fs::read(bios_path)
            .map_err(|e| format!("Failed to read BIOS file {}: {e}", bios_path.display()))?;
        Some(data)
    } else {
        None
    };
    ...
    Atari5200::new(&config)
}

fn make_system(cli: &CliArgs) -> Atari5200 {
...
mod tests {
    ...
    fn cli_parser_reads_basic_options() {
        ...
        assert_eq!(cli.bios_path, Some(PathBuf::from("5200.rom")));
        assert!(cli.headless);
        assert_eq!(cli.region, Atari5200Region::Pal);
    ...
    fn cli_parser_rejects_unknown_args() {
        let result = parse_cli(&["emu-atari-5200", "--bogus"]);
