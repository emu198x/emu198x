...
#![allow(dead_code)] // Constants will be used as more instructions are implemented.

// Condition code register (CCR) flags - bits 0-4

/// Carry flag - set if operation produced a carry or borrow.
...
/// Negative flag - set if result is negative (MSB set).
pub const N: u16 = 0x0008;

/// Extend flag - copy of C for multi-precision arithmetic.
/// Not affected by MOVE, EXG, LEA, etc.
pub const X: u16 = 0x0010;

// System byte flags - bits 8-15
...
/// Supervisor mode - when set, CPU is in supervisor mode.
pub const S: u16 = 0x2000;

/// Trace mode - when set, trace exception after each instruction.
...
impl Status {
    ...
    /// Set a flag if condition is true.
    #[must_use]
    ...
    pub fn condition(sr: u16, cc: u8) -> bool {
        match cc & 0x0F {
            ...
            0xB => (sr & N) != 0,                           // MI (minus)
            0xC => {
                // GE (greater or equal) - N and V same
                let n = (sr & N) != 0;
                let v = (sr & V) != 0;
                n == v
            }
            0xD => {
                // LT (less than) - N and V different
                let n = (sr & N) != 0;
                let v = (sr & V) != 0;
                n != v
            }
            0xE => {
                // GT (greater than) - Z clear and (N and V same)
                let z = (sr & Z) != 0;
                let n = (sr & N) != 0;
                let v = (sr & V) != 0;
                !z && (n == v)
            ...
            0xF => {
                // LE (less or equal) - Z set or (N and V different)
                let z = (sr & Z) != 0;
                let n = (sr & N) != 0;
                let v = (sr & V) != 0;
                z || (n != v)
