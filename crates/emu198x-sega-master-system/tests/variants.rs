...
impl Fixture {
    ...
    fn command(&self) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_emu198x-sega-master-system"));
...
fn output(command: &mut Command) -> Value {
    let output = command.output().expect("launch sega-master-system");
    assert!(
...
fn launch_and_script_select_both_regions_without_losing_the_cartridge() {
    ...
    for model in runtime_sega_master_system::Model::ALL {
        let report = output(
            fixture
                .script(json!([
                    {"action":"set_machine", "machine":model.variant_id()},
                    {"action":"memory_read", "addr":0, "len":1},
                    {"action":"reset"},
                    {"action":"memory_read", "addr":0, "len":1}
                ]))
    ...
    for args in [vec!["--cart", path.as_str()], vec![path.as_str()]] {
        let responses = mcp(
            ...
            &[
                ...
                json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"sega-master-system-pal"}}}),
                json!({"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"unknown"}}}),
                json!({"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"memory_read","arguments":{"addr":0,"len":1}}}),
            ],
        );
        assert!(
        ...
        assert_eq!(
            ...
            runtime_sega_master_system::Model::SmsPal.profile_id()
        );
...
fn mcp_switch_uses_cartridge_bytes_after_the_source_file_is_removed() {
    ...
    let read = json!({"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"memory_read","arguments":{"addr":0,"len":1}}});
    ...
    let switch = json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"sega-master-system-pal"}}});
    writeln!(
        stdin,
        "{switch}
{read}"
...
#[test]
fn mcp_restores_battery_save_before_switching_models() {
    let fixture = Fixture::new();
    std::fs::write(fixture.0.join("game.sav"), vec![0x42; 32768]).expect("SRAM sidecar");
    let path = fixture.0.join("game.rom").display().to_string();
    let replies = mcp(
        &fixture,
        &[path.as_str()],
        &[
            json!({"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"sms-japan-ntsc"}}}),
            json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"poke_byte","arguments":{"addr":65532,"value":8}}}),
            json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"memory_read","arguments":{"addr":33059,"len":1}}}),
        ],
    );
    assert_eq!(
        tool_body(&replies[0])["profile_id"],
        "sega-master-system-japan-ntsc"
    );
    tool_body(&replies[1]);
    assert_eq!(tool_body(&replies[2])["bytes"], json!([0x42]));
}
