...
Options:
    ...
    --variant KIND  sms-ntsc | sms-pal | game-gear [default: sms-ntsc]
...
Controls:
    ...
    Z / X           buttons 1 and 2
    Enter           Pause (SMS) / Start (Game Gear)

Examples:
    emu198x-sega-master-system --cart sonic.sms
    emu198x-sega-master-system --cart game.gg --variant game-gear --scale 4
";

/// Console variant — selects the model, frame tick budget, refresh rate, the
/// display aspect, and whether Enter is the SMS Pause or the Game Gear Start.
...
/// The Sega Master System / Game Gear as a [`UiSystem`] for the shared harness.
...
impl UiSystem for SmsSystem {
    ...
    fn window_title(&self) -> String {
        match self.variant {
            Variant::GameGear => "Emu198x Sega Game Gear".to_owned(),
            _ => "Emu198x Sega Master System".to_owned(),
        }
    }
    ...
    // The SMS drove a 4:3 TV; the Game Gear is a square-pixel LCD.
    fn display_aspect_ratio(&self) -> Option<f32> {
        match self.variant {
            Variant::GameGear => None,
            _ => Some(4.0 / 3.0),
        }
    }
    ...
    fn map_keys(&self, code: KeyCode) -> Option<&'static [&'static str]> {
        // The single console button: SMS Pause vs Game Gear Start. The runtime
        // takes both as named key events.
        match code {
            KeyCode::Enter | KeyCode::NumpadEnter => Some(match self.variant {
                Variant::GameGear => &["start"],
                _ => &["pause"],
            }),
            _ => None,
        }
...
pub fn run(cli: Cli) -> Result<(), String> {
    ...
    let runtime = SmsRuntime::new(cli.variant.model(), cart);
        ...
        match arg.as_str() {
            ...
            "--variant" => {
                cli.variant = match next_arg(&mut iter, "--variant").as_str() {
                    ...
                    other => die(&format!(
                        "--variant expects sms-ntsc|sms-pal|game-gear, got {other}"
                    )),
                };
            }
            "--scale" => {
...
mod tests {
    ...
    #[test]
    fn pad_maps_and_console_button_follows_variant() {
        let sms = SmsSystem {
            variant: Variant::SmsNtsc,
        };
