...
impl Fixture {
    ...
    fn command(&self) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_emu198x-colecovision"));
        // The file convention must work without HOME or installed ROMs.
        command.env_clear();
        command.env("EMU198X_COLECO_BIOS", self.0.join("monitor.rom"));
        command
    }
...
fn output(command: &mut Command) -> Value {
    let output = command.output().expect("launch colecovision");
    assert!(
...
fn launch_and_script_select_both_regions_without_losing_the_cartridge() {
    ...
    for model in runtime_coleco_colecovision::Model::ALL {
        let report = output(
            fixture
                .script(json!([
                    {"action":"set_machine", "machine":model.variant_id()},
                    {"action":"memory_read", "addr":32768, "len":1},
                    {"action":"reset"},
                    {"action":"memory_read", "addr":32768, "len":1}
                ]))
    ...
    for args in [vec!["--cart", path.as_str()], vec![path.as_str()]] {
        let responses = mcp(
            ...
            &[
                ...
                json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"memory_read","arguments":{"addr":32768,"len":1}}}),
                json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"coleco-colecovision-pal"}}}),
                json!({"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"unknown"}}}),
                json!({"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"memory_read","arguments":{"addr":32768,"len":1}}}),
            ],
        );
        assert!(
        ...
        assert_eq!(
            ...
            runtime_coleco_colecovision::Model::CvPal.profile_id()
        );
...
fn mcp_switch_uses_cartridge_bytes_after_the_source_file_is_removed() {
    ...
    let read = json!({"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"memory_read","arguments":{"addr":32768,"len":1}}});
    ...
    let switch = json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"coleco-colecovision-pal"}}});
    writeln!(
        stdin,
        "{switch}
{read}"
...
fn firmware_options_and_errors_use_the_shared_resolver() {
    let fixture = Fixture::new();
    std::fs::copy(
        fixture.0.join("monitor.rom"),
        fixture.0.join("colecovision.rom"),
    )
    .expect("conventional name");
    for directory_flag in [false, true] {
        let mut command = fixture.script(json!([]));
        command.env_remove("EMU198X_COLECO_BIOS");
        if directory_flag {
            command.arg("--rom-dir").arg(&fixture.0);
        } else {
            command.env("EMU198X_COLECO_ROM_DIR", &fixture.0);
        }
        assert_eq!(output(&mut command)["bios_loaded"], true);
    }
    for spec in [
        ...
        assert_eq!(
            output(
                fixture
                    .script(json!([]))
                    .env("EMU198X_COLECO_BIOS", "missing.rom")
                    .args(["--rom", &spec])
            )["bios_loaded"],
            true
        );
    ...
    for mode in ["--headless", "--mcp"] {
        for args in [
            ...
            let result = fixture
                .command()
                .env_remove("EMU198X_COLECO_BIOS")
                .arg(mode)
                .args(args)
                .output()
                .expect("launch");
...
#[test]
fn mcp_rejects_invalid_bios_even_when_supplied_by_environment() {
    let fixture = Fixture::new();
    let invalid = fixture.0.join("invalid.rom");
    std::fs::write(&invalid, [0; 3]).expect("invalid BIOS");
    for mode in ["--mcp", "--headless"] {
        for path in [&invalid, &fixture.0.join("absent.rom")] {
            let result = fixture
                .command()
                .env("EMU198X_COLECO_BIOS", path)
                .arg(mode)
                .output()
                .expect("launch");
