//! ColecoVision-specific MCP tools — Z80 CPU snapshot, VDP snapshot,
//! and raw memory observation. Sits beside the shared
//! `register_common_tools` surface.
...
fn tool_query_cpu(_args: Value, session: &mut CvSession) -> Result<Value, ToolError> {
...
fn tool_memory_read(args: Value, session: &mut CvSession) -> Result<Value, ToolError> {
    ...
    for offset in 0..len {
        let byte = cv.peek(addr.wrapping_add(offset as u16));
        if offset > 0 {
            hex.push(' ');
        }
...
/// Register ColecoVision-specific tools.
pub fn register_cv_tools(registry: &mut ToolRegistry<CvSession>) {
    fn add(
        registry: &mut ToolRegistry<CvSession>,
        name: &'static str,
        description: &'static str,
        schema: Value,
        run: fn(Value, &mut CvSession) -> Result<Value, ToolError>,
    ...
    add(
        registry,
        "query_cpu",
        "Z80 register snapshot (A/F/BC/DE/HL/IX/IY/SP/PC, IFF1/IFF2, IM, halt, total cycles).",
        empty(),
        tool_query_cpu,
    );
    add(
