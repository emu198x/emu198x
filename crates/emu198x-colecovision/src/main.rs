...
//!
//! Three modes: UI (default), headless script (`--bios`/`--cart`/
//! `--frames`/`--screenshot`/`--audio-capture`/`--script`) and MCP
//! (`--mcp` / `--mcp-stdio`). Building with `--no-default-features` drops the
...
/// shared with the UI, so a bare `--cart game.col` opens the interactive window.
...
mod tests {
    ...
    fn detect_mode_treats_bare_cart_as_ui() {
        let args = vec!["--cart".to_owned(), "game.col".to_owned()];
    ...
    fn detect_mode_recognises_script_via_automation_flags() {
        for flag in ["--script", "--frames", "--screenshot", "--audio-capture"] {
            let args = vec!["--cart".to_owned(), "game.col".to_owned(), flag.to_owned()];
            assert_eq!(
                detect_mode(&args),
                Mode::Script,
