//! Snapshot/restore round-trip coverage for the Atari 7800 runtime.
//!
...
fn run_to(runtime: &mut Atari7800Runtime, target: u64) {
    runtime
        .run_until(
...
fn snapshot_round_trip_preserves_live_state() {
    let mut runtime = Atari7800Runtime::blank(MODEL);
    run_to(&mut runtime, WARMUP);

    let snap = runtime.snapshot().expect("blank runtime should snapshot");

    let mut restored = Atari7800Runtime::blank(MODEL);
