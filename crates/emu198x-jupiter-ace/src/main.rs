//! Jupiter Ace headless runner — minimal first-port binary.
...
use machine_jupiter_ace::{JupiterAce, FB_HEIGHT, FB_WIDTH};

fn usage() {
    eprintln!(
        "\
Usage: emu198x-jupiter-ace --rom PATH [OPTIONS]

Required:
    --rom PATH                8 KB Forth ROM

Options:
    --ram-kb N                base RAM in KB (1 / 3 / 16 / 48) [default: 3]
        ...
        match arg.as_str() {
            ...
            "--ram-kb" => {
                cli.ram_kb = next_value()?
                    .parse()
                    .map_err(|e| format!("--ram-kb expects a positive integer: {e}"))?;
            }
            "--frames" => {
...
fn run(cli: Cli) -> Result<(), String> {
    ...
    let mut sys = JupiterAce::new(rom, cli.ram_kb * 1024)?;
    for _ in 0..cli.frames {
        sys.run_frame();
    }
    println!(
        "Jupiter Ace runtime: master_clock={} frames={} ram_kb={}",
        sys.master_clock(),
        sys.frame_count(),
        cli.ram_kb
    );
