//! `emu198x-sega-sg-1000` — Sega SG-1000 native binary.
//!
//! Three modes: UI (default), headless script (`--cart`/`--frames`/
//! `--screenshot`/`--audio-capture`/`--script`) and MCP (`--mcp`/`--mcp-stdio`).
...
/// mode so existing invocations keep working; `--cart`/`--region` are shared
/// with the UI, so a bare `--cart game.sg` opens the interactive window.
...
mod tests {
    ...
    fn detect_mode_treats_bare_cart_as_ui() {
        let args = vec!["--cart".to_owned(), "game.sg".to_owned()];
    ...
    fn detect_mode_recognises_script_via_automation_flags() {
        for flag in ["--script", "--frames", "--screenshot", "--audio-capture"] {
            let args = vec!["--cart".to_owned(), "game.sg".to_owned(), flag.to_owned()];
            assert_eq!(
                detect_mode(&args),
                Mode::Script,
