//! Headless SG-1000 runner — `--cart` / `--frames` / shared `--script`.
...
Cartridge:
    --cart PATH                cartridge ROM (required for any work)
...
fn run_cli(cli: Cli) -> Result<serde_json::Value, String> {
    ...
    let runtime = Sg1000Runtime::new(cli.region.model(), cart_bytes);
