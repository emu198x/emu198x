...
impl Fixture {
    ...
    fn command(&self) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_emu198x-sega-sg-1000"));
...
fn output(command: &mut Command) -> Value {
    let output = command.output().expect("launch sega-sg-1000");
    assert!(
...
fn launch_and_script_select_both_regions_without_losing_the_cartridge() {
    ...
    for model in runtime_sega_sg_1000::Model::ALL {
        let report = output(
            fixture
                .script(json!([
    ...
    for args in [vec!["--cart", path.as_str()], vec![path.as_str()]] {
        let responses = mcp(
            ...
            &[
                ...
                json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"sega-sg-1000-pal"}}}),
        ...
        assert_eq!(
            ...
            runtime_sega_sg_1000::Model::Sg1000Pal.profile_id()
        );
...
fn mcp_switch_uses_cartridge_bytes_after_the_source_file_is_removed() {
    ...
    let switch = json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"sega-sg-1000-pal"}}});
    writeln!(
        stdin,
        "{switch}
{read}"
