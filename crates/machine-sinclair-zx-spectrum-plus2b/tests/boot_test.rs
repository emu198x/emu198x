//! Integration boot test for the ZX Spectrum +2B.
//!
...
//! ~5 seconds of CPU time (250 frames), and asserts that screen RAM
//! contains the boot menu pixels.
//!
...
fn boot_to_menu_renders_screen_content() {
    ...
    assert!(
        nonzero > 50,
        "+2B should boot to menu with screen content (got {nonzero} non-zero bytes in screen RAM)"
    );
}
