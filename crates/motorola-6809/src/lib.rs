...
mod tests {
    ...
    fn assert_opcode_traps(bytes: &[u8], label: fmt::Arguments<'_>) {
        let mut memory = [0x12; 0x10000];
        memory[0x4000..0x4000 + bytes.len()].copy_from_slice(bytes);
        let mut cpu = cpu_at(0x4000);
        ...
        assert!(cpu.halt, "{label} did not trip illegal-opcode diagnostic");
    }

    fn assert_primary_opcode_slot_dispatches(opcode: u8) {
        match opcode {
            0x10 => assert_opcode_dispatches(&[0x10, 0x3F], format_args!("primary $10 prefix")),
            0x11 => assert_opcode_dispatches(&[0x11, 0x3F], format_args!("primary $11 prefix")),
            _ => assert_opcode_dispatches(&[opcode], format_args!("primary ${opcode:02X}")),
        }
    }
    ...
    const OFFICIAL_PRIMARY_OPCODE_PAGE: &[u8] = &[
        0x00, 0x03, 0x04, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0C, 0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12,
        0x13, 0x16, 0x17, 0x19, 0x1A, 0x1C, 0x1D, 0x1E, 0x1F, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25,
        0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34,
        0x35, 0x36, 0x37, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3F, 0x40, 0x43, 0x44, 0x46, 0x47, 0x48,
        0x49, 0x4A, 0x4C, 0x4D, 0x4F, 0x50, 0x53, 0x54, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x5C, 0x5D,
        0x5F, 0x60, 0x63, 0x64, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x6C, 0x6D, 0x6E, 0x6F, 0x70, 0x73,
        0x74, 0x76, 0x77, 0x78, 0x79, 0x7A, 0x7C, 0x7D, 0x7E, 0x7F, 0x80, 0x81, 0x82, 0x83, 0x84,
        0x85, 0x86, 0x88, 0x89, 0x8A, 0x8B, 0x8C, 0x8D, 0x8E, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95,
        ...
        0xC3, 0xC4, 0xC5, 0xC6, 0xC8, 0xC9, 0xCA, 0xCB, 0xCC, 0xCE, 0xD0, 0xD1, 0xD2, 0xD3, 0xD4,
    ...
    const OFFICIAL_PREFIX_10_OPCODE_PAGE: &[u8] = &[
    ...
    fn official_opcode_map_does_not_trip_illegal_opcode_diagnostic() {
        for &opcode in OFFICIAL_PRIMARY_OPCODE_PAGE {
            assert_primary_opcode_slot_dispatches(opcode);
        }
        for &opcode in OFFICIAL_PREFIX_10_OPCODE_PAGE {
    ...
    #[test]
    fn official_opcode_pages_match_documented_and_unused_slots() {
        for opcode in 0x00..=0xFF {
            if OFFICIAL_PRIMARY_OPCODE_PAGE.contains(&opcode) {
                assert_primary_opcode_slot_dispatches(opcode);
            } else {
                assert_opcode_traps(&[opcode], format_args!("primary ${opcode:02X}"));
            }

            if OFFICIAL_PREFIX_10_OPCODE_PAGE.contains(&opcode) {
                assert_opcode_dispatches(&[0x10, opcode], format_args!("10 ${opcode:02X}"));
            } else {
                assert_opcode_traps(&[0x10, opcode], format_args!("10 ${opcode:02X}"));
            }

            if OFFICIAL_PREFIX_11_OPCODE_PAGE.contains(&opcode) {
                assert_opcode_dispatches(&[0x11, opcode], format_args!("11 ${opcode:02X}"));
            } else {
                assert_opcode_traps(&[0x11, opcode], format_args!("11 ${opcode:02X}"));
            }
        }
    }
