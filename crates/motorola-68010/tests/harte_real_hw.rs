...
//! `Cpu68010` wrapper. Most of the 68010 ISA is inherited from the
...
//!
//! The selected 68000 source subset contains 911,343 rows. This harness
//! excludes every structurally identified 68000 address-error row because the
//! current 68010 wrapper does not yet implement the 68010 Format `$8`
//! bus/address-error frame. Every remaining row must agree exactly. Counts,
//! read/write partitions, and a source-row fingerprint pin the exclusion
//! boundary.
//!
//! # Expected divergence
//!
...
//!   the fixture: the 68010 pushes a 4-word (8-byte) frame with a
//!   Format/Vector word, whereas the 68000 pushes a 3-word (6-byte)
...
const EXPECTED_INCLUDED_FIXTURES: usize = 113;
const EXPECTED_SOURCE_ROWS: usize = 911_343;
const EXPECTED_COMPARED_ROWS: usize = 753_676;
const EXPECTED_EXACT_ROWS: usize = 753_676;
const EXPECTED_EXCLUDED_ADDRESS_ERROR_ROWS: usize = 157_667;
const EXPECTED_EXCLUDED_READ_ADDRESS_ERROR_ROWS: usize = 149_908;
const EXPECTED_EXCLUDED_WRITE_ADDRESS_ERROR_ROWS: usize = 7_759;
const EXPECTED_EXCLUSION_FINGERPRINT: ExclusionFingerprint =
    ExclusionFingerprint(0xC623_26B7_6F88_9408);
...
impl ExclusionFingerprint {
    ...
    fn write_case(&mut self, case_name: &str, kind: AddressErrorKind) {
...
fn address_error_tail_matches(test: &serde_json::Map<String, Value>) -> Option<()> {
...
fn classify_address_error(test: &Value) -> Option<AddressErrorKind> {
    ...
    address_error_tail_matches(test)?;
...
fn compare_final(cpu: &Cpu68010, mem: &SparseMem, final_state: &Value) -> Vec<Mismatch> {
...
/// 68000 corpus and the 68010. Skipped entirely from the pass-rate
...
fn inherited_subset_passes_real_hw_68000_corpus() {
    ...
    println!();
    println!(
        "68010 inherited-subset cross-check against SingleStepTests 68000 corpus ({} fixtures total):",
        entries.len()
    );
    ...
    for path in &entries {
        ...
        for test in &tests {
            ...
            if let Some(kind) = classify_address_error(test) {
                ...
                exclusion_fingerprint.write_case(&case_name, kind);
                continue;
            }
        ...
        let excluded = excluded_read_address_errors + excluded_write_address_errors;
    ...
    assert_eq!(
        total_source_rows,
        total_compared_rows + total_excluded_address_errors,
