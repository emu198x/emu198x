...
fn a_nop_costs_four_tstates() {
    // 8 KB of `NOP` in place of the BIOS, no expansion RAM, no cartridge.
    let mut aquarius = Aquarius::new(vec![0x00u8; 8 * 1024], 0, AquariusRegion::Ntsc);
    ...
    let tstates_before = aquarius.cpu_tstates();
    let retired_before = aquarius.z80_instructions_retired();
    for _ in 0..4_000 {
        aquarius.step_tick();
    }
    let tstates = aquarius.cpu_tstates() - tstates_before;
    let retired = aquarius.z80_instructions_retired() - retired_before;
