...
fn a_nop_costs_four_tstates() {
    // 8 KB of `NOP` in place of the BIOS, no cartridge; the Z80 starts at
    // `$0000`.
    let mut cv = ColecoVision::new(vec![0x00u8; 8 * 1024], Vec::new(), CvRegion::Ntsc);
    ...
    let tstates_before = cv.cpu_cycles();
    let retired_before = cv.z80_instructions_retired();
    for _ in 0..4_000 {
        cv.step_tick();
    }
    let tstates = cv.cpu_cycles() - tstates_before;
    let retired = cv.z80_instructions_retired() - retired_before;
