...
impl Fixture {
    fn new() -> Self {
        ...
        std::fs::write(dir.join("game.rom"), vec![0x5a; 4096]).expect("cartridge");
        Self(dir)
    }

    fn command(&self) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_emu198x-atari-2600"));
...
fn output(command: &mut Command) -> Value {
    let output = command.output().expect("launch atari-2600");
    assert!(
...
fn launch_and_script_select_both_regions_without_losing_the_cartridge() {
    ...
    for model in runtime_atari_2600::Model::ALL {
        let report = output(
            fixture
                .script(json!([
                    {"action":"set_machine", "machine":model.variant_id()},
                    {"action":"memory_read", "addr":4096, "len":1},
                    {"action":"reset"},
                    {"action":"memory_read", "addr":4096, "len":1}
                ]))
    ...
    for args in [vec!["--cart", path.as_str()], vec![path.as_str()]] {
        let responses = mcp(
            ...
            &[
                ...
                json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"memory_read","arguments":{"addr":4096,"len":1}}}),
                json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"atari-2600-pal"}}}),
                json!({"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"unknown"}}}),
                json!({"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"memory_read","arguments":{"addr":4096,"len":1}}}),
            ],
        );
        assert!(
        ...
        assert_eq!(
            ...
            runtime_atari_2600::Model::Vcs2600Pal.profile_id()
        );
...
fn mcp_switch_uses_cartridge_bytes_after_the_source_file_is_removed() {
    ...
    let read = json!({"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"memory_read","arguments":{"addr":4096,"len":1}}});
    ...
    let switch = json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"atari-2600-pal"}}});
    writeln!(
        stdin,
        "{switch}
{read}"
