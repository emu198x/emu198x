//! TZX / CDT block parsing.
//!
//! One format under two names: the Spectrum calls the file a `.tzx`, the
//! Amstrad CPC calls it a `.cdt`, and the bytes are the same. This crate holds
//! the block table and nothing else — no system knows about it and it knows
//! about no system, which is the condition
//! `knowledge/decisions/crate-naming.md` sets for a format crate to lose its
//! system prefix.
//!
//! # The clock this speaks in
//!
//! **Spans come out in the format's reference T-states, not any machine's.**
//! TZX quotes every pulse length against a 3.5 MHz clock, and it does so in a
//! CDT read by a 4 MHz Amstrad exactly as in a TZX read by a Spectrum. So the
//! parser converts nothing: a Spectrum's crate takes the stream unchanged
//! because its clock *is* the reference, and the CPC's scales by 40/35 —
//! Caprice32's `CYCLE_SCALE`.
//!
//! Putting that scale here would mean asking the parser which machine is
//! reading, and a parser that asks is one that should have stayed two crates.
//!
//! Source references:
//! - `knowledge/concepts/tape-formats.md`
//! - Promoted from `format-sinclair-zx-spectrum-tzx` on 2026-08-15

use common_tape::{TapeSpan, append_pause_spans, push_pulse};
...
fn parse_standard_speed(
    ...
    append_pause_spans(pause_ms, TSTATES_PER_MS, current_level, stream);
