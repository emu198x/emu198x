...
impl Cpu68000 {
    ...
    pub(crate) fn perform_shift(
        ...
        // The real 68000 clears C and X when the register shift count
        // exceeds the operand width (8/16/32). For count == width, C/X
        // reflect the last bit shifted out normally. For count > width,
        // they're unconditionally cleared. Verified against Tom Harte.
    ...
    /// between real hardware and Musashi; only `V` (the "undefined"
    /// flag in PRM) differs. Dispatch on
...
// ─── BCD ALU helpers — real-hardware and Musashi variants ─────────
//
// The result and carry agree between the two; only the "undefined"
// V flag is computed differently. Real-hw values match the upstream
// Tom Harte SingleStepTests/680x0 corpus (which was generated by
// single-stepping a physical 68000). Musashi values match the
// m68k-test-gen 68010 / 68020 corpora (which use Musashi as oracle).

/// Real-hardware BCD add. V = bit 7 of `(~uncorrected) & corrected_result`.
...
/// Real-hardware BCD subtract. V = bit 7 of `uncorrected & ~result`.
