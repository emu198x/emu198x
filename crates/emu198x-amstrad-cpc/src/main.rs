//! `emu198x-amstrad-cpc` — Amstrad CPC464 native binary.
//!
//! One binary, three modes: UI (default), headless script, and MCP. `main.rs`
//! is a tiny dispatcher; the modes live in `src/ui.rs`, `src/script.rs`, and
//! `src/mcp.rs`. Building with `--no-default-features` drops the `ui` feature
//! (winit + wgpu) for the headless screenshot / script pipeline.
...
/// mode so existing invocations keep working; `--rom` and `--tape` are shared
/// with the UI, so `--rom cpc464.rom --tape game.cdt` opens the window.
...
mod tests {
    ...
    #[test]
    fn detect_mode_treats_a_bare_rom_and_tape_as_ui() {
        let args = vec![
            "--rom".to_owned(),
            "cpc464.rom".to_owned(),
            "--tape".to_owned(),
    ...
    fn detect_mode_recognises_script_via_automation_flags() {
        for flag in [
            ...
            let args = vec!["--rom".to_owned(), "cpc464.rom".to_owned(), flag.to_owned()];
            assert_eq!(
                detect_mode(&args),
                Mode::Script,
    ...
    fn detect_mode_recognises_mcp() {
        ...
        assert_eq!(detect_mode(&["--mcp-stdio".to_owned()]), Mode::Mcp);
    }

    #[test]
    fn mcp_wins_over_script_flags() {
        // Both present is a malformed invocation; serving MCP is the safer
        // reading, because a script mode that then blocks on stdin looks like
