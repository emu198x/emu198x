...
fn execute_int_im2(z80: &mut Z80) {
    ...
    match exec_count {
        0 => {
            ...
            z80.walker.staged.push_val = z80.regs.pc;
            // Vector table address = (I << 8) | vector latched by IntAck.
            let vector_addr = ((z80.regs.i as u16) << 8) | z80.walker.staged.data_lo as u16;
...
fn execute_nmi(z80: &mut Z80) {
    let exec_count = count_executes_before(z80.walker.sequence, z80.walker.step_idx);
    if exec_count == 0 {
        // After NmiAck: stage push of current PC, jump to 0x0066
        z80.walker.staged.push_val = z80.regs.pc;
        z80.regs.pc = 0x0066;
        z80.regs.wz = 0x0066;
    }
