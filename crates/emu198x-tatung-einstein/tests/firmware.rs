...
impl Fixture {
    ...
    fn command(&self) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_emu198x-tatung-einstein"));
        // The file convention must work without HOME or installed ROMs.
        command.env_clear();
        command.env("EMU198X_EINSTEIN_MOS", self.0.join("monitor.rom"));
        command
    }
...
fn output(command: &mut Command) -> Value {
    let output = command.output().expect("launch tatung-einstein");
    assert!(
...
fn firmware_options_and_errors_use_the_shared_resolver() {
    ...
    std::fs::copy(fixture.0.join("monitor.rom"), fixture.0.join("mos.rom"))
        .expect("conventional name");
    for directory_flag in [false, true] {
        let mut command = fixture.script(json!([]));
        command.env_remove("EMU198X_EINSTEIN_MOS");
        if directory_flag {
            command.arg("--rom-dir").arg(&fixture.0);
        } else {
            command.env("EMU198X_EINSTEIN_ROM_DIR", &fixture.0);
        }
    ...
    for spec in [
        ...
        assert_eq!(
            output(
                fixture
                    .script(json!([]))
                    .env("EMU198X_EINSTEIN_MOS", "missing.rom")
                    .args(["--rom", &spec])
            )["mos_loaded"],
            true
        );
    ...
    for mode in ["--headless", "--mcp"] {
        for args in [
            ...
            let result = fixture
                .command()
                .env_remove("EMU198X_EINSTEIN_MOS")
                .arg(mode)
                .args(args)
                .output()
                .expect("launch");
    ...
    let bad = fixture
        .command()
        .env("EMU198X_EINSTEIN_MOS", fixture.0.join("bad.rom"))
        .arg("--mcp")
        .output()
        .expect("invalid MCP");
