//! The Scorpion never withholds the CPU clock.
//!
//! `ScorpionUla::tick` assigns `cpu_clock = true` with the comment "No
...
fn the_cpu_clock_is_never_withheld_across_two_whole_frames() {
    ...
    assert_eq!(
        withheld, 0,
        "the Scorpion withheld the CPU clock on {withheld} of {ticks} ticks \
...
fn the_frame_timing_declares_no_contention_pattern() {
    let ula = ScorpionUla::new();
    assert_eq!(
        ula.frame_timing().contention_pattern,
        [0; 8],
        "the Scorpion's declared contention pattern must stay empty",
    );
}
