...
struct InlineTool {
    ...
    schema: Value,
    run: fn(Value, &mut Zx81Session) -> Result<Value, ToolError>,
}

impl Tool<Zx81Session> for InlineTool {
    fn name(&self) -> &str {
        self.name
    }
...
fn zx_ref(s: &Zx81Session) -> Result<&Zx81, ToolError> {
    s.machine()
        .machine()
...
fn tool_query_cpu(_args: Value, session: &mut Zx81Session) -> Result<Value, ToolError> {
...
fn tool_memory_read(args: Value, session: &mut Zx81Session) -> Result<Value, ToolError> {
...
pub fn register_zx81_tools(registry: &mut ToolRegistry<Zx81Session>) {
    fn add(
        registry: &mut ToolRegistry<Zx81Session>,
        name: &'static str,
        description: &'static str,
        schema: Value,
        run: fn(Value, &mut Zx81Session) -> Result<Value, ToolError>,
