...
/// mode so existing invocations keep working; `--rom` is shared with the UI, so
/// a bare `--rom zx81.rom` opens the interactive window.
fn is_script_flag(arg: &str) -> bool {
    matches!(
        arg,
        "--script" | "--frames" | "--screenshot" | "--audio-capture" | "--ram-bytes"
    )
}
...
mod tests {
    ...
    fn detect_mode_treats_bare_rom_as_ui() {
        let args = vec!["--rom".to_owned(), "zx81.rom".to_owned()];
    ...
    fn detect_mode_recognises_script_via_automation_flags() {
        for flag in ["--script", "--frames", "--screenshot", "--audio-capture"] {
            let args = vec!["--rom".to_owned(), "zx81.rom".to_owned(), flag.to_owned()];
            assert_eq!(
                detect_mode(&args),
                Mode::Script,
