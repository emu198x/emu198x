...
impl Cpu68000 {
        ...
        if matches!(self.state, State::Idle) {
            ...
            if matches!(self.state, State::Idle) {
                if let Some(op) = self.micro_ops.pop() {
                    ...
                    else if let MicroOp::Internal(cycles) = op { self.state = State::Internal { cycles }; }
                }
            }
        }
        ...
        while let Some(op) = self.micro_ops.front() {
            if op.is_instant() {
                ...
                match op {
                    ...
                    MicroOp::PromoteIRC => { self.start_next_instruction(); }
    ...
    fn decode_and_execute(&mut self) {
        ...
        match opcode {
            0x4E71 => { /* NOP */ }
            0x4E70 => { if self.regs.is_supervisor() { self.micro_ops.push(MicroOp::AssertReset); self.micro_ops.push(MicroOp::Internal(124)); } else { self.state = State::Halted; } }
    ...
    fn continue_instruction(&mut self) {
        match self.followup_tag {
            ...
            FOLLOWUP_MOVE_READ_SRC_EA_LONG => { let lo = self.consume_irc(); self.addr |= u32::from(lo); self.followup_tag = FOLLOWUP_MOVE_READ_SRC_DATA; self.micro_ops.push(MicroOp::Execute); }
            FOLLOWUP_MOVE_CALC_DST_EA_LONG => { let lo = self.consume_irc(); self.addr |= u32::from(lo); self.followup_tag = FOLLOWUP_MOVE_WRITE_DATA; self.micro_ops.push(MicroOp::Execute); }
            FOLLOWUP_MOVE_READ_SRC_DATA_LONG => {
                let lo = self.consume_irc(); self.data |= u32::from(lo);
                let is_alu = match self.ir & 0xF000 { 0xD000 | 0x9000 | 0xB000 | 0xC000 | 0x8000 => true, _ => false };
                if is_alu { self.followup_tag = FOLLOWUP_ALU_CALC_DST; } else { self.followup_tag = FOLLOWUP_MOVE_CALC_DST_EA; }
                self.micro_ops.push(MicroOp::Execute);
            }
            FOLLOWUP_MOVE_READ_SRC_EA_DISP => { let disp = self.consume_irc() as i16 as i32; self.addr = self.regs.a(self.ea_reg as usize).wrapping_add(disp as u32); self.followup_tag = FOLLOWUP_MOVE_READ_SRC_DATA; self.micro_ops.push(MicroOp::Execute); }
            FOLLOWUP_MOVE_CALC_DST_EA_DISP => { let disp = self.consume_irc() as i16 as i32; self.addr = self.regs.a(self.ea_reg as usize).wrapping_add(disp as u32); self.followup_tag = FOLLOWUP_MOVE_WRITE_DATA; self.micro_ops.push(MicroOp::Execute); }
            FOLLOWUP_MOVE_READ_SRC_EA_PCDISP => { let disp = self.consume_irc() as i16 as i32; self.addr = self.ea_pc.wrapping_add(disp as u32); self.followup_tag = FOLLOWUP_MOVE_READ_SRC_DATA; self.micro_ops.push(MicroOp::Execute); }
            FOLLOWUP_MOVE_CALC_DST_EA_PCDISP => { let disp = self.consume_irc() as i16 as i32; self.addr = self.ea_pc.wrapping_add(disp as u32); self.followup_tag = FOLLOWUP_MOVE_WRITE_DATA; self.micro_ops.push(MicroOp::Execute); }
            FOLLOWUP_MOVE_READ_SRC_DATA => {
                let src_mode = self.src_mode.unwrap();
                match src_mode {
                    AddrMode::DataReg(reg) => { self.data = self.regs.d[reg as usize]; self.followup_tag = FOLLOWUP_MOVE_CALC_DST_EA; self.micro_ops.push(MicroOp::Execute); }
                    AddrMode::AddrReg(reg) => { self.data = self.regs.a[reg as usize]; self.followup_tag = FOLLOWUP_MOVE_CALC_DST_EA; self.micro_ops.push(MicroOp::Execute); }
                    AddrMode::Immediate => { let val = self.consume_irc(); if self.size == Size::Long { self.data = u32::from(val) << 16; self.followup_tag = FOLLOWUP_MOVE_READ_SRC_DATA_LONG; } else { self.data = u32::from(val); self.followup_tag = FOLLOWUP_MOVE_CALC_DST_EA; } self.micro_ops.push(MicroOp::Execute); }
                    _ => { self.followup_tag = FOLLOWUP_MOVE_CALC_DST_EA; self.queue_read_ops(self.size); self.micro_ops.push(MicroOp::Execute); }
                }
            }
            FOLLOWUP_MOVE_CALC_DST_EA => { if self.calc_ea(self.dst_mode.unwrap(), self.size) { self.followup_tag = FOLLOWUP_MOVE_WRITE_DATA; } self.micro_ops.push(MicroOp::Execute); }
            FOLLOWUP_MOVE_WRITE_DATA => {
                let dst_mode = self.dst_mode.unwrap();
                match dst_mode {
                    AddrMode::DataReg(reg) => {
                        let val = self.data; self.set_flags_move(val, self.size); let reg_val = &mut self.regs.d[reg as usize];
                        *reg_val = match self.size { Size::Byte => (*reg_val & 0xFFFF_FF00) | (val & 0xFF), Size::Word => (*reg_val & 0xFFFF_0000) | (val & 0xFFFF), Size::Long => val };
                        self.in_followup = false;
                    }
                    AddrMode::AddrReg(reg) => { let val = self.data; self.regs.a[reg as usize] = if self.size == Size::Word { (val as i16 as i32) as u32 } else { val }; self.in_followup = false; }
                    _ => { self.set_flags_move(self.data, self.size); self.queue_write_ops(self.size); self.in_followup = false; }
                }
            }
            FOLLOWUP_ALU_READ_SRC => {
                let src_mode = self.src_mode.unwrap();
                match src_mode {
                    AddrMode::DataReg(reg) => { self.data = self.regs.d[reg as usize]; self.followup_tag = FOLLOWUP_ALU_CALC_DST; self.micro_ops.push(MicroOp::Execute); }
                    AddrMode::AddrReg(reg) => { self.data = self.regs.a[reg as usize]; self.followup_tag = FOLLOWUP_ALU_CALC_DST; self.micro_ops.push(MicroOp::Execute); }
                    AddrMode::Immediate => { let val = self.consume_irc(); if self.size == Size::Long { self.data = u32::from(val) << 16; self.followup_tag = FOLLOWUP_MOVE_READ_SRC_DATA_LONG; } else { self.data = u32::from(val); self.followup_tag = FOLLOWUP_ALU_CALC_DST; } self.micro_ops.push(MicroOp::Execute); }
    ...
    fn calc_ea(&mut self, mode: AddrMode, _size: Size) -> bool {
        match mode {
            ...
            AddrMode::AddrInd(reg) => { self.addr = self.regs.a(reg as usize); true }
            AddrMode::AddrIndPostInc(reg) => { self.addr = self.regs.a(reg as usize); let step = if reg == 7 && self.size == Size::Byte { 2 } else { self.size.bytes() as u32 }; self.regs.set_a(reg as usize, self.addr.wrapping_add(step)); true }
            AddrMode::AddrIndPreDec(reg) => { let step = if reg == 7 && self.size == Size::Byte { 2 } else { self.size.bytes() as u32 }; self.addr = self.regs.a(reg as usize).wrapping_sub(step); self.regs.set_a(reg as usize, self.addr); true }
            ...
            AddrMode::AbsShort => { self.addr = (self.consume_irc() as i16 as i32) as u32; true }
            AddrMode::AbsLong => {
                let hi = self.consume_irc(); self.addr = u32::from(hi) << 16;
                self.followup_tag = match self.followup_tag {
    ...
    fn finish_bus_cycle(&mut self, op: MicroOp, read_data: u16) {
        match op {
            ...
            MicroOp::ReadWord | MicroOp::ReadByte | MicroOp::InterruptAck => { self.data = u32::from(read_data); }
            _ => {}
        }
    }
    ...
    fn check_condition(&self, cond: u8) -> bool {
        ...
        let sr = self.regs.sr; let n = sr & N != 0; let z = sr & Z != 0; let v = sr & V != 0; let c = sr & C != 0;
        match cond & 0x0F {
    ...
    fn exec_alu(&mut self, op: AluOp, src: u32, dst: u32, size: Size) -> u32 {
        let mask = size.mask(); let s = src & mask; let d = dst & mask;
        match op {
    ...
    fn set_flags_add(&mut self, s: u32, d: u32, r: u32, size: Size) {
        ...
        self.regs.sr &= !(N | Z | V | C | X); let msb = size.msb_mask();
        if r == 0 { self.regs.sr |= Z; } if r & msb != 0 { self.regs.sr |= N; }
    ...
    fn set_flags_logic(&mut self, r: u32, size: Size) {
        use crate::flags::{N, Z, V, C};
        self.regs.sr &= !(N | Z | V | C); let msb = size.msb_mask();
        if r == 0 { self.regs.sr |= Z; } if r & msb != 0 { self.regs.sr |= N; }
    }

    fn set_flags_move(&mut self, val: u32, size: Size) {
        use crate::flags::{N, Z, V, C};
        self.regs.sr &= !(N | Z | V | C); let mask = size.mask(); let msb = size.msb_mask(); let v = val & mask;
        if v == 0 { self.regs.sr |= Z; } if v & msb != 0 { self.regs.sr |= N; }
    }
}
