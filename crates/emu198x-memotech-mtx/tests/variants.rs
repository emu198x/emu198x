...
impl Fixture {
    ...
    fn command(&self) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_emu198x-memotech-mtx"));
        // The file convention must work without HOME or installed ROMs.
        command.env_clear();
        command.env("EMU198X_MTX_ROM", self.0.join("monitor.rom"));
        command
    }
...
fn output(command: &mut Command) -> Value {
    let output = command.output().expect("launch memotech-mtx");
    assert!(
...
fn launch_and_script_select_every_existing_preset() {
    ...
    for model in runtime_memotech_mtx::Model::ALL {
        let report = output(
            fixture
                .script(json!([
...
fn firmware_options_and_errors_use_the_shared_resolver() {
    let fixture = Fixture::new();
    std::fs::copy(fixture.0.join("monitor.rom"), fixture.0.join("mtx.rom"))
        .expect("conventional name");
    for directory_flag in [false, true] {
        let mut command = fixture.script(json!([]));
        command.env_remove("EMU198X_MTX_ROM");
        if directory_flag {
            command.arg("--rom-dir").arg(&fixture.0);
        } else {
            command.env("EMU198X_MTX_ROM_DIR", &fixture.0);
        }
    ...
    for spec in [
        ...
        format!(
            "{}={}",
            runtime_memotech_mtx::ROM_FIRMWARE_ID,
        ...
        assert_eq!(
            output(
                fixture
                    .script(json!([]))
                    .env("EMU198X_MTX_ROM", "missing.rom")
                    .args(["--rom", &spec])
            )["rom_loaded"],
            true
        );
    ...
    let bad = fixture
        .command()
        .env("EMU198X_MTX_ROM", fixture.0.join("bad.rom"))
        .arg("--mcp")
        .output()
        .expect("invalid MCP");
...
fn mcp_publishes_and_executes_the_shared_switch_tool() {
    ...
    for request in [
        ...
        json!({"jsonrpc":"2.0", "id":2, "method":"tools/call", "params":{"name":"set_machine", "arguments":{"machine":"mtx512"}}}),
    ] {
    ...
    assert_eq!(
        body["profile_id"],
        runtime_memotech_mtx::Model::Mtx512.profile_id(),
        "{switched}"
    );
}
