...
struct InlineTool {
    ...
    schema: Value,
    run: fn(Value, &mut MtxSession) -> Result<Value, ToolError>,
}

impl Tool<MtxSession> for InlineTool {
    fn name(&self) -> &str {
        self.name
    }
    ...
    fn call(&self, arguments: Value, session: &mut MtxSession) -> Result<ToolResponse, ToolError> {
...
fn mtx_ref(s: &MtxSession) -> Result<&Mtx, ToolError> {
    s.machine()
        .machine()
...
fn tool_query_cpu(_args: Value, session: &mut MtxSession) -> Result<Value, ToolError> {
    let mtx = mtx_ref(session)?;
    let r = &mtx.cpu().regs;
    Ok(json!({
        "af": format!("${:04X}", r.af),
        "bc": format!("${:04X}", r.bc),
...
fn tool_memory_read(args: Value, session: &mut MtxSession) -> Result<Value, ToolError> {
    ...
    for offset in 0..len {
        let byte = mtx.peek_memory(addr.wrapping_add(offset as u16));
        if offset > 0 {
            hex.push(' ');
        }
...
pub fn register_mtx_tools(registry: &mut ToolRegistry<MtxSession>) {
    fn add(
        registry: &mut ToolRegistry<MtxSession>,
        name: &'static str,
        description: &'static str,
        schema: Value,
        run: fn(Value, &mut MtxSession) -> Result<Value, ToolError>,
