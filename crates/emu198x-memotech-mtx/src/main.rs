...
/// the UI, so a bare `--model mtx512` opens the interactive window.
...
mod tests {
    ...
    fn detect_mode_treats_bare_model_as_ui() {
        let args = vec!["--model".to_owned(), "mtx512".to_owned()];
    ...
    fn detect_mode_recognises_script_via_automation_flags() {
        for flag in ["--script", "--frames", "--screenshot", "--audio-capture"] {
            let args = vec!["--rom".to_owned(), "mtx.rom".to_owned(), flag.to_owned()];
            assert_eq!(
                detect_mode(&args),
                Mode::Script,
