...
fn usage() {
    eprintln!(
        "\
Usage: emu198x-memotech-mtx --rom PATH [OPTIONS]

Required:
    --rom PATH                16 KB ROM (8 KB OS + 8 KB BASIC)

Options:
    --model NAME              mtx500 (32 KB RAM) | mtx512 (64 KB RAM) [default: mtx500]
        ...
        match arg.as_str() {
            ...
            "--model" => {
                cli.model = match next_value()?.as_str() {
                    "mtx500" => MtxModel::Mtx500,
                    "mtx512" => MtxModel::Mtx512,
                    other => {
                        return Err(format!(
                            "--model expects mtx500 or mtx512, got {other}"
                        ));
                    }
                };
            }
...
fn run(cli: Cli) -> Result<(), String> {
    ...
    let mut sys = Mtx::new(rom, cli.model)?;
    for _ in 0..cli.frames {
        sys.run_frame();
    }
    println!(
        "MTX runtime: master_clock={} frames={} model={:?}",
        sys.master_clock(),
        sys.frame_count(),
        cli.model
    );
...
mod tests {
    ...
    fn parse_cli_defaults() {
        ...
        assert!(matches!(cli.model, MtxModel::Mtx500));
    }

    #[test]
    fn parse_cli_accepts_mtx512() {
        let argv = vec!["--model".into(), "mtx512".into()];
        let cli = parse_cli(argv).expect("ok");
        assert!(matches!(cli.model, MtxModel::Mtx512));
    }
}
