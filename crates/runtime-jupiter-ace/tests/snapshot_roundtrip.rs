//! Snapshot/restore round-trip coverage for the Jupiter Ace runtime.
//!
...
const MODEL: Model = Model::Ace3k;
const WARMUP: u64 = 50_000;
const LOCKSTEP: u64 = 100_000;

fn run_to(runtime: &mut JupiterAceRuntime, target: u64) {
    runtime
        .run_until(
...
fn snapshot_round_trip_preserves_live_state() {
    let mut runtime = JupiterAceRuntime::blank(MODEL);
    run_to(&mut runtime, WARMUP);

    let snap = runtime.snapshot().expect("blank runtime should snapshot");

    let mut restored = JupiterAceRuntime::blank(MODEL);
