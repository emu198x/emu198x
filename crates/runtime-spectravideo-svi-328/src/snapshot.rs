//! Postcard-encoded snapshot envelope for the SVI-328 runtime.
//!
//! Serialises the **live machine** (Z80, TMS9918 VDP, AY-3-8910 PSG, 8255 PPI,
//! ROM, RAM, and banking) so a restore resumes exactly, rather than the old
//! bootstrap envelope that cold-booted from the ROM/cart. Mirrors the SG-1000 /
...
#[derive(Serialize)]
struct SviRuntimeSnapshotRefV2<'a> {
    version: u16,
    time: u64,
    model_id: &'a str,
    machine: Option<&'a Svi328>,
}

/// Owning envelope used during decode.
#[derive(Deserialize)]
struct SviRuntimeSnapshotV2 {
    version: u16,
    time: u64,
    model_id: String,
...
pub(crate) fn encode(runtime: &Svi328Runtime) -> Result<Vec<u8>, MachineError> {
    let snapshot = SviRuntimeSnapshotRefV2 {
...
pub(crate) fn decode(runtime: &mut Svi328Runtime, bytes: &[u8]) -> Result<(), MachineError> {
    let snapshot: SviRuntimeSnapshotV2 =
...
#[cfg(test)]
mod tests {
    use super::{SviRuntimeSnapshotRefV2, decode};
    ...
    fn decode_rejects_unsupported_version() {
        ...
        let bytes = postcard::to_allocvec(&SviRuntimeSnapshotRefV2 {
            version: 999,
            time: 0,
            model_id: runtime.model().model_id(),
            machine: None,
