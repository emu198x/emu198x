//! C64-specific MCP tools: `save_disk`, `save_disk9`, `save_tape`,
//! `set_port_drive`.
//!
//! The shared registrars cover the machine-agnostic surface (run frames,
//! query, media, capture, reset, …) and, from the profile's
//! `basic-program-load` / `tape-autoload` capabilities, the
//! `load_basic_program` and `autoload_tape` tools over the runtime's
//! `MachineCore` loader hooks. `save_disk` carries the writeback half of
//! the Code198x BASIC-authoring flow; `set_port_drive` exposes the
//! runtime's per-port IEC drive selector.
...
use runtime_commodore_c64::{C64Runtime, C64SessionQueryProvider, DriveKind};
