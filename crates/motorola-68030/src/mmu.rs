...
pub fn check_tt_040(
    ...
    match fc {
        2 | 6 => tt_match_040(itt0, addr, is_supervisor)
            .or_else(|| tt_match_040(itt1, addr, is_supervisor)),
        _ => tt_match_040(dtt0, addr, is_supervisor)
    ...
    fn process_walk_descriptor_030(
        ...
        match dt {
            ...
            DescriptorType030::Pointer4 => {
                ...
                let index =
                    (ctx.pending.logical_addr >> ctx.remaining_shift) & ((1u32 << field_width) - 1);
...
pub fn walk_040(
    ...
    match page_pdt {
        ...
        3 => {
            ...
            return finish_040_page(real_desc, page_offset, page_mask, write_protect, is_write);
        }
        _ => unreachable!(),
    }
...
#[cfg(test)]
mod tests {
    #![allow(clippy::identity_op, clippy::too_many_arguments, clippy::unwrap_used)]
    ...
    fn make_tt040(addr_base: u8, addr_mask: u8, enable: bool, s_field: u8, cm: u8, w: bool) -> u32 {
        (u32::from(addr_base) << 24)
            | (u32::from(addr_mask) << 16)
            | if enable { 1 << 15 } else { 0 }
    ...
    fn walk030_invalid_descriptor_faults() {
        ...
        let mem = TestMemory::new(0x2_0000);
        let table_base = 0x1_0000u32;
    ...
    fn walk030_write_protect_accumulates() {
        ...
        match result {
            WalkResult030::Ok { write_protect, .. } => {
                assert!(
                    write_protect,
                    "WP should accumulate from 8-byte pointer level"
                );
            }
            WalkResult030::Fault { .. } => panic!("unexpected fault"),
        }
    ...
    fn walk040_identity_4kb() {
        ...
        let result = walk_040(&tc, root_table, 0x0000_1234, 5, false, |a| mem.read_long(a));
        match result {
    ...
    fn walk040_remapped() {
        ...
        let result = walk_040(&tc, root_table, 0x0000_1ABC, 5, false, |a| mem.read_long(a));
        match result {
            WalkResult040::Ok { physical_addr, .. } => {
    ...
    fn walk040_write_protect() {
        ...
        // Read should succeed.
        let result = walk_040(&tc, root_table, 0x0000_0000, 5, false, |a| mem.read_long(a));
        assert!(matches!(
            result,
            WalkResult040::Ok {
                write_protect: true,
        ...
        // Write should fault.
        let result = walk_040(&tc, root_table, 0x0000_0000, 5, true, |a| mem.read_long(a));
        assert!(matches!(result, WalkResult040::Fault { .. }));
    }
