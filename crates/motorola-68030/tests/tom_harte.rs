...
//! of the 68000 fixture format — the 68030-only registers (`msp`,
//! `vbr`, `cacr`, `caar`) live in extra fields that are zero on 68000
//! fixtures.
//!
//! Default corpus root:
//!   `~/Projects/198x/assets/test-suites/m68k-generated/m68030/v1/`
//!
...
//!
//! ```sh
//! cargo test -p motorola-68030 --test tom_harte -- --ignored --nocapture
//! ```
...
struct CpuState {
    ...
    prefetch: [u16; 2],
    ram: Vec<(u32, u8)>,
    // 68030+ supersets — absent on 68000 files, default to zero.
    #[serde(default)]
    msp: u32,
    #[serde(default)]
    vbr: u32,
...
fn candidate_fixture_roots() -> Vec<PathBuf> {
    ...
    if let Ok(path) = std::env::var("M68030_TEST_DATA") {
        roots.push(PathBuf::from(path));
    }
    roots.push(home.join("Projects/198x/assets/test-suites/m68k-generated/m68030/v1"));
    roots
}
...
fn fixture_root() -> PathBuf {
    candidate_fixture_roots()
        ...
        .unwrap_or_else(|| {
            let home = std::env::var("HOME").expect("HOME set");
            PathBuf::from(home).join("Projects/198x/assets/test-suites/m68k-generated/m68030/v1")
        })
}
...
fn apply_initial(cpu: &mut Cpu68030, mem: &mut SparseMem, initial: &CpuState) {
...
fn service_bus(cpu: &mut Cpu68030, mem: &mut SparseMem) {
...
fn run_one_instruction(cpu: &mut Cpu68030, mem: &mut SparseMem) -> bool {
...
fn compare_final(cpu: &Cpu68030, mem: &SparseMem, final_state: &CpuState) -> Vec<Mismatch> {
    let mut v = Vec::new();
...
fn harte_baseline_full_sweep() {
    ...
    if !root.exists() {
        emu198x_test_skip::skip!(
            "SingleStepTests 680030 corpus not staged: {}",
            root.display()
        );
    }
...
/// the corpus and should pass on the type-alias 68030 today. If this
...
fn harte_nop_smoke() {
    ...
    assert_eq!(
        r.passed, r.total,
        "NOP should pass on Cpu68000-aliased Cpu68030 — harness wiring is broken if it doesn't",
    );
}
