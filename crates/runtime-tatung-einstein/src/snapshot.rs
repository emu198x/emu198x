//! Postcard-encoded snapshot envelope for the Einstein runtime.
//!
//! Serialises the **live machine** (Z80, TMS9918 VDP, AY-3-8910 PSG, WD1770 FDC
//! with any inserted disks, RAM and ROM) so a restore resumes exactly, rather
//! than the old bootstrap envelope that cold-booted from the ROM. Mirrors the
//! SG-1000 / Game Boy shape: a borrowing envelope for encode (no clone), an
//! owning envelope for decode.
...
#[derive(Serialize)]
struct EinsteinRuntimeSnapshotRefV2<'a> {
    version: u16,
    time: u64,
    model_id: &'a str,
    machine: Option<&'a Einstein>,
}

/// Owning envelope used during decode.
#[derive(Deserialize)]
struct EinsteinRuntimeSnapshotV2 {
    version: u16,
    time: u64,
    model_id: String,
...
pub(crate) fn encode(runtime: &EinsteinRuntime) -> Result<Vec<u8>, MachineError> {
    let snapshot = EinsteinRuntimeSnapshotRefV2 {
...
pub(crate) fn decode(runtime: &mut EinsteinRuntime, bytes: &[u8]) -> Result<(), MachineError> {
    let snapshot: EinsteinRuntimeSnapshotV2 =
...
#[cfg(test)]
mod tests {
    use super::{EinsteinRuntimeSnapshotRefV2, decode};
    ...
    fn decode_rejects_unsupported_version() {
        ...
        let bytes = postcard::to_allocvec(&EinsteinRuntimeSnapshotRefV2 {
            version: 999,
            time: 0,
            model_id: runtime.model().model_id(),
            machine: None,
