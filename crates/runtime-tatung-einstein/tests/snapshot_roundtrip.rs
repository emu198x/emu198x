//! Snapshot/restore round-trip coverage for the Tatung Einstein runtime.
//!
...
const MODEL: Model = Model::Einstein;
const WARMUP: u64 = 50_000;
const LOCKSTEP: u64 = 100_000;

fn run_to(runtime: &mut EinsteinRuntime, target: u64) {
    runtime
        .run_until(
...
fn snapshot_round_trip_preserves_live_state() {
    let mut runtime = EinsteinRuntime::blank(MODEL);
    run_to(&mut runtime, WARMUP);

    let snap = runtime.snapshot().expect("blank runtime should snapshot");

    let mut restored = EinsteinRuntime::blank(MODEL);
