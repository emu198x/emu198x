...
struct InlineTool {
    ...
    schema: Value,
    run: fn(Value, &mut Vic20Session) -> Result<Value, ToolError>,
}

impl Tool<Vic20Session> for InlineTool {
    fn name(&self) -> &str {
        self.name
    }
...
fn vic_ref(s: &Vic20Session) -> Result<&Vic20, ToolError> {
    s.machine()
        .machine()
...
fn tool_query_cpu(_args: Value, session: &mut Vic20Session) -> Result<Value, ToolError> {
    let vic = vic_ref(session)?;
    let r = &vic.cpu().regs;
    Ok(json!({
        "a":  format!("${:02X}", r.a),
        "x":  format!("${:02X}", r.x),
        ...
        "pc": format!("${:04X}", r.pc),
        "p":  format!("${:02X}", r.p),
        "master_clock": vic.master_clock(),
    }))
}

fn tool_memory_read(args: Value, session: &mut Vic20Session) -> Result<Value, ToolError> {
    ...
    for offset in 0..len {
        let byte = vic.peek_memory(addr.wrapping_add(offset as u16));
        if offset > 0 {
            hex.push(' ');
        }
...
pub fn register_vic20_tools(registry: &mut ToolRegistry<Vic20Session>) {
    fn add(
        registry: &mut ToolRegistry<Vic20Session>,
        name: &'static str,
        description: &'static str,
        schema: Value,
        run: fn(Value, &mut Vic20Session) -> Result<Value, ToolError>,
