//! `emu198x-commodore-vic-20` — VIC-20 native binary.
//!
...
/// mode so existing invocations keep working; the ROM/`--region` flags are
/// shared with the UI, so a bare `--region ntsc` opens the interactive window.
...
mod tests {
    ...
    #[test]
    fn detect_mode_treats_bare_region_as_ui() {
        let args = vec!["--region".to_owned(), "ntsc".to_owned()];
