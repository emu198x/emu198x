//! Amiga custom chip register file.
//!
//! The custom chip registers live at $DFF000-$DFF1FF. This module provides
//! the central storage and dispatch for reads/writes. Individual chip
//! modules (Agnus, Denise, Paula, Copper, Blitter) own their state, but
//! the register file routes accesses to them.
...
mod tests {
    ...
    fn set_clr_set_mode() {
        let mut reg = 0u16;
        set_clr_write(&mut reg, 0x8003); // Set bits 0,1
    ...
    fn set_clr_clear_mode() {
        ...
        set_clr_write(&mut reg, 0x0003); // Clear bits 0,1
    ...
    fn set_clr_preserves_other_bits() {
        ...
        set_clr_write(&mut reg, 0x8100); // Set bit 8
