...
//!
//! Adapted from `Emu198x-Oldest/crates/mos-riot-6532` (port 2026-06-01)
//! as a foundation for Atari 2600 and 7800. Self-contained, no external
//! dependencies.
//!
...
impl Riot6532 {
    ...
    /// Mutable access to RAM (for debug writes).
    pub fn ram_mut(&mut self) -> &mut [u8; 128] {
        &mut self.ram
    }
    ...
    /// Serialize RIOT state for save states.
    #[must_use]
    pub fn save_state(&self) -> Vec<u8> {
        let mut data = Vec::with_capacity(128 + 16);
        data.extend_from_slice(&self.ram);
        data.push(self.port_a);
        data.push(self.ddr_a);
        data.push(self.input_a);
        data.push(self.port_b);
        data.push(self.ddr_b);
        data.push(self.input_b);
        data.push(self.timer);
        data.extend_from_slice(&self.divider.to_le_bytes());
        data.extend_from_slice(&self.prescaler.to_le_bytes());
        data.push(u8::from(self.underflow));
        data.push(u8::from(self.post_underflow));
        data
    }

    /// Restore RIOT state from a save state.
    ///
    /// # Errors
    ///
    ...
    pub fn load_state(&mut self, data: &[u8]) -> Result<usize, String> {
        if data.len() < 128 + 11 {
            return Err("RIOT state truncated".into());
        }
        let mut p = 0;
