...
impl Fixture {
    ...
    fn command(&self) -> Command {
        let mut command = Command::new(env!("CARGO_BIN_EXE_emu198x-spectravideo-svi-328"));
        // The file convention must work without HOME or installed ROMs.
        command.env_clear();
        command.env("EMU198X_SVI_328_BIOS", self.0.join("monitor.rom"));
        command
    }
...
fn output(command: &mut Command) -> Value {
    let output = command.output().expect("launch spectravideo-svi-328");
    assert!(
...
fn launch_and_script_select_every_existing_preset() {
    ...
    for model in runtime_spectravideo_svi_328::Model::ALL {
        let report = output(
            fixture
                .script(json!([
...
fn firmware_options_and_errors_use_the_shared_resolver() {
    let fixture = Fixture::new();
    std::fs::copy(fixture.0.join("monitor.rom"), fixture.0.join("svi-328.rom"))
        .expect("conventional name");
    for directory_flag in [false, true] {
        let mut command = fixture.script(json!([]));
        command.env_remove("EMU198X_SVI_328_BIOS");
        if directory_flag {
            command.arg("--rom-dir").arg(&fixture.0);
        } else {
            command.env("EMU198X_SVI_328_ROM_DIR", &fixture.0);
        }
    ...
    for spec in [
        ...
        format!(
            "{}={}",
            runtime_spectravideo_svi_328::BIOS_FIRMWARE_ID,
        ...
        assert_eq!(
            output(
                fixture
                    .script(json!([]))
                    .env("EMU198X_SVI_328_BIOS", "missing.rom")
                    .args(["--rom", &spec])
            )["bios_loaded"],
            true
        );
    ...
    for mode in ["--headless", "--mcp"] {
        for args in [
            ...
            let result = fixture
                .command()
                .env_remove("EMU198X_SVI_328_BIOS")
                .arg(mode)
                .args(args)
                .output()
                .expect("launch");
    ...
    let bad = fixture
        .command()
        .env("EMU198X_SVI_328_BIOS", fixture.0.join("bad.rom"))
        .arg("--mcp")
        .output()
        .expect("invalid MCP");
...
fn mcp_publishes_and_executes_the_shared_switch_tool() {
    ...
    for request in [
        ...
        json!({"jsonrpc":"2.0", "id":2, "method":"tools/call", "params":{"name":"set_machine", "arguments":{"machine":"spectravideo-svi-328-pal"}}}),
    ] {
    ...
    assert_eq!(
        body["profile_id"],
        runtime_spectravideo_svi_328::Model::Svi328Pal.profile_id(),
        "{switched}"
    );
}
...
fn mcp_firmware_flags_never_become_cartridges() {
    ...
    for spec in [
        ...
        let replies = mcp(
            ...
            &["--rom", &spec],
            &[
                json!({"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"query","arguments":{"path":"bios.loaded"}}}),
...
fn cartridge_load_reset_and_switch_are_consistent_across_scripts_and_mcp() {
    ...
    let report = output(
        fixture
            .script(json!([{"action":"set_machine", "machine":"spectravideo-svi-328-pal"}]))
            .arg("--cart")
            .arg(&path),
    );
    assert_eq!(
    ...
    let replies = mcp(
        ...
        &[
            ...
            json!({"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"set_machine","arguments":{"machine":"spectravideo-svi-328-pal"}}}),
    ...
    assert_eq!(
        tool_body(&replies[3])["profile_id"],
        "spectravideo-svi-328-pal"
    );
