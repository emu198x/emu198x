//! `emu198x-spectravideo-svi-328` — SVI-328 native binary.
//!
...
/// mode so existing invocations keep working; `--bios`/`--cart`/`--region` are
/// shared with the UI, so a bare `--cart game.rom` opens the interactive window.
