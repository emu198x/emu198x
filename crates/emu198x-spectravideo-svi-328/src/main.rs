//! Spectravideo SVI-328 headless runner — minimal first-port binary.
...
fn usage() {
    eprintln!(
        "\
Usage: emu198x-spectravideo-svi-328 --bios PATH [OPTIONS]

Required:
    --bios PATH               SVI-328 system ROM (32 KB BASIC + OS)
...
BIOS source (when --bios is omitted):
    1. EMU198X_SVI_328_BIOS env var
    2. ~/.emu198x/roms/spectravideo-svi-328/svi-328.rom

Examples:
    emu198x-spectravideo-svi-328 \\
        --bios ~/.emu198x/roms/spectravideo-svi-328/svi-328.rom \\
        --frames 300 --screenshot svi-boot.png
"
    );
}
...
fn run(cli: Cli) -> Result<(), String> {
    ...
    let mut sys = Svi328::new(bios, cli.region);
    if let Some(cart_path) = cli.cart.as_deref() {
        ...
        if cart.len() > 0x4000 {
            return Err(format!(
                "cart at {} is {} bytes; SVI-328 cart ceiling is 16 KB",
                cart_path.display(),
                cart.len()
            ));
        }
    ...
    println!(
        "SVI-328 runtime: tstates={} frames={}",
        sys.cpu_tstates(),
        sys.frame_count()
    );
...
mod tests {
    ...
    fn parse_cli_defaults() {
        ...
        assert!(matches!(cli.region, SviRegion::Ntsc));
    ...
    fn parse_cli_accepts_full_flags() {
        let argv = vec![
            "--bios".to_string(),
            "/tmp/svi.rom".to_string(),
            "--cart".to_string(),
            "/tmp/game.bin".to_string(),
            "--region".to_string(),
            "pal".to_string(),
            "--frames".to_string(),
            "60".to_string(),
        ...
        assert_eq!(cli.bios.unwrap(), Path::new("/tmp/svi.rom"));
        assert_eq!(cli.cart.unwrap(), Path::new("/tmp/game.bin"));
        assert!(matches!(cli.region, SviRegion::Pal));
