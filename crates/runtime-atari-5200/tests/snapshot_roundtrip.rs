//! Snapshot/restore round-trip coverage for the Atari 5200 runtime.
//!
...
fn run_to(runtime: &mut Atari5200Runtime, target: u64) {
    runtime
        .run_until(
...
fn snapshot_round_trip_preserves_live_state() {
    let mut runtime = Atari5200Runtime::blank(MODEL);
    run_to(&mut runtime, WARMUP);

    let snap = runtime.snapshot().expect("blank runtime should snapshot");

    let mut restored = Atari5200Runtime::blank(MODEL);
