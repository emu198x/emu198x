...
mod tests {
    ...
    fn cli_parser_allows_empty_rom_in_script_mode() {
        let cli = parse_cli(&["amiga-runner", "--script", "boot.json"], None)
            .expect("parse should succeed")
            .expect("help was not requested");
    ...
    fn cli_parser_derives_chipset_from_selected_model() {
        let ecs = parse_cli(
            &["amiga-runner", "--rom", "kick.rom", "--model", "a500plus"],
            None,
        )
        .expect("parse should succeed")
        .expect("help was not requested");
        let aga = parse_cli(
            &["amiga-runner", "--rom", "kick.rom", "--model", "a1200"],
            None,
        )
        .expect("parse should succeed")
        .expect("help was not requested");
    ...
    fn cli_parser_rejects_removed_chipset_override() {
        let result = parse_cli(
            &["amiga-runner", "--rom", "kick.rom", "--chipset", "ecs"],
            None,
        );
