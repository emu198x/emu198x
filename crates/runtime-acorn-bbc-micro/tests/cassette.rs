//! Cassette `load_media` wiring for the BBC Micro (#386).
//!
//! Drives the runtime's UEF tape path end to end with a synthetic 16 KiB MOS, so
...
fn runtime() -> BbcMicroRuntime {
    BbcMicroRuntime::new(Model::BbcModelB, vec![0u8; 16384])
        .expect("synthetic MOS builds the machine")
}

fn load(rt: &mut BbcMicroRuntime, bytes: &[u8]) -> Result<(), MachineError> {
