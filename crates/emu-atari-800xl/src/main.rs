//! Atari 800XL emulator binary.
//!
//! Runs the 800XL with a winit window and wgpu renderer, or in
...
const FB_WIDTH: u32 = gtia::FB_WIDTH;
...
struct CliArgs {
    ...
    os_rom_path: Option<PathBuf>,
    basic_rom_path: Option<PathBuf>,
    headless: bool,
    frames: u32,
...
fn parse_args_from(args: &[String]) -> Result<Option<CliArgs>, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--os-rom" => {
                cli.os_rom_path = Some(next_option_value(args, &mut i, "--os-rom")?);
            }
            "--basic-rom" => {
                cli.basic_rom_path = Some(next_option_value(args, &mut i, "--basic-rom")?);
            }
            "--basic" => {
                cli.basic_enabled = true;
            }
            "--headless" => {
                cli.headless = true;
            }
...
impl App {
    fn new(system: Atari800xl, menu: Menu, menu_ids: MenuIds) -> Self {
        let frame_duration = match system.region() {
            Atari800xlRegion::Ntsc => FRAME_DURATION_NTSC,
            Atari800xlRegion::Pal => FRAME_DURATION_PAL,
        };
        Self {
            system,
            window: None,
    ...
    fn handle_key(&mut self, keycode: KeyCode, pressed: bool) {
        if let Some(input) = input_map::map_keycode(keycode) {
            match input {
                Atari800xlInput::P0Fire => {
                    self.system.set_fire(pressed);
                }
                Atari800xlInput::P0Up => {
                    self.joy_up = pressed;
                    self.system.set_joystick(
                        self.joy_up, self.joy_down, self.joy_left, self.joy_right,
                    );
                }
                Atari800xlInput::P0Down => {
                    self.joy_down = pressed;
                    self.system.set_joystick(
                        self.joy_up, self.joy_down, self.joy_left, self.joy_right,
                    );
                }
                Atari800xlInput::P0Left => {
                    self.joy_left = pressed;
                    self.system.set_joystick(
                        self.joy_up, self.joy_down, self.joy_left, self.joy_right,
                    );
                }
                Atari800xlInput::P0Right => {
                    self.joy_right = pressed;
                    self.system.set_joystick(
                        self.joy_up, self.joy_down, self.joy_left, self.joy_right,
                    );
                }
                Atari800xlInput::Start => {
                    self.start_pressed = pressed;
                    self.system.set_console_keys(
                        self.start_pressed,
                        self.select_pressed,
                        self.option_pressed,
                    );
                }
                Atari800xlInput::Select => {
                ...
                Atari800xlInput::Break => {
                    // BREAK key not yet wired to POKEY
                }
            }
        }
    }
...
fn make_system_result(cli: &CliArgs) -> Result<Atari800xl, String> {
    let rom_data = if let Some(ref rom_path) = cli.rom_path {
    ...
    let os_rom = if let Some(ref path) = cli.os_rom_path {
        let data = std::fs::read(path)
            .map_err(|e| format!("Failed to read OS ROM {}: {e}", path.display()))?;
        Some(data)
    } else {
        None
    };

    let basic_rom = if let Some(ref path) = cli.basic_rom_path {
        let data = std::fs::read(path)
            .map_err(|e| format!("Failed to read BASIC ROM {}: {e}", path.display()))?;
        Some(data)
    } else {
        None
    };
    ...
    Atari800xl::new(&config)
}

fn make_system(cli: &CliArgs) -> Atari800xl {
    match make_system_result(cli) {
        Ok(system) => {
            if let Some(ref rom_path) = cli.rom_path {
                eprintln!("Loaded ROM: {}", rom_path.display());
            }
            if let Some(ref path) = cli.os_rom_path {
                eprintln!("Loaded OS ROM: {}", path.display());
            }
            if let Some(ref path) = cli.basic_rom_path {
                eprintln!("Loaded BASIC ROM: {}", path.display());
            }
            system
        }
        Err(e) => {
...
fn main() {
    ...
    if cli.rom_path.is_none() && cli.os_rom_path.is_none() {
        eprintln!("No ROM files specified. Use --rom and/or --os-rom.");
        print_usage();
        process::exit(1);
    }
...
mod tests {
    ...
    fn cli_parser_reads_basic_options() {
        ...
        assert_eq!(cli.rom_path, Some(PathBuf::from("game.bin")));
        assert_eq!(cli.os_rom_path, Some(PathBuf::from("atarixl.rom")));
        assert_eq!(cli.basic_rom_path, Some(PathBuf::from("ataribas.rom")));
        assert!(cli.basic_enabled);
        assert!(cli.headless);
        assert_eq!(cli.region, Atari800xlRegion::Pal);
    ...
    fn cli_parser_rejects_unknown_args() {
        let result = parse_cli(&["emu-atari-800xl", "--bogus"]);
    ...
    fn cli_parser_help_returns_none() {
        assert!(
            parse_cli(&["emu-atari-800xl", "--help"])
    ...
    #[test]
    fn cli_defaults() {
        let cli = parse_cli(&["emu-atari-800xl", "--headless"])
            .expect("parse should succeed")
            .expect("help was not requested");

        assert_eq!(cli.rom_path, None);
        assert_eq!(cli.os_rom_path, None);
        assert_eq!(cli.basic_rom_path, None);
        assert!(!cli.basic_enabled);
        assert!(cli.headless);
        assert_eq!(cli.region, Atari800xlRegion::Ntsc);
