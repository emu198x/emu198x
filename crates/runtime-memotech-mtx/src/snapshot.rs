//! Postcard-encoded snapshot envelope for the MTX runtime.
//!
//! Serialises the **live machine** (Z80, TMS9918 VDP, SN76489 PSG, Z80 CTC,
//! RAM, paging register, and keyboard) so a restore resumes exactly, rather
//! than the old bootstrap envelope that cold-booted from the ROM. Mirrors the
//! Sord M5 shape: a borrowing envelope for encode (no clone), an owning
//! envelope for decode.
...
#[derive(Serialize)]
struct MtxRuntimeSnapshotRefV2<'a> {
    version: u16,
    time: u64,
    model_id: &'a str,
    machine: Option<&'a Mtx>,
}

/// Owning envelope used during decode.
#[derive(Deserialize)]
struct MtxRuntimeSnapshotV2 {
    version: u16,
    time: u64,
    model_id: String,
    machine: Option<Mtx>,
...
pub(crate) fn encode(runtime: &MtxRuntime) -> Result<Vec<u8>, MachineError> {
    let snapshot = MtxRuntimeSnapshotRefV2 {
...
pub(crate) fn decode(runtime: &mut MtxRuntime, bytes: &[u8]) -> Result<(), MachineError> {
    let snapshot: MtxRuntimeSnapshotV2 =
...
#[cfg(test)]
mod tests {
    use super::{MtxRuntimeSnapshotRefV2, decode};
    ...
    fn decode_rejects_unsupported_version() {
        let mut runtime = MtxRuntime::blank(Model::Mtx500);
        let bytes = postcard::to_allocvec(&MtxRuntimeSnapshotRefV2 {
            version: 999,
            time: 0,
            model_id: runtime.model().model_id(),
            machine: None,
