...
fn a_nop_costs_four_tstates() {
    // 8 KB of `NOP` in place of the monitor ROM, no cartridge.
    let mut m5 = SordM5::new(vec![0x00u8; 8 * 1024], Vec::new(), M5Region::Pal);
    ...
    let tstates_before = m5.cpu_tstates();
    let retired_before = m5.z80_instructions_retired();
    for _ in 0..4_000 {
        m5.step_tick();
    }
    let tstates = m5.cpu_tstates() - tstates_before;
    let retired = m5.z80_instructions_retired() - retired_before;
