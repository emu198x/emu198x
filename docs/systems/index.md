...
clock; relaxed models are honest debt). *Drive* = `win+mcp` (native window,
primary tier) / `mcp` (headless, `--script`+`--mcp`) / `—` (no binary). *Net* =
the standalone internet-capability verdict.

## Primary (native window, CPU oracles green)

| System | Boot today | Timing | Drive | Net |
|--------|-----------|--------|-------|-----|
...
| [Commodore PET](commodore/pet.md) | BASIC `READY.` | relaxed (1x CRTC) | mcp | Marginal |
| [Jupiter Ace](jupiter/ace.md) | Forth; interactive | hc-ish; e-o-f display | mcp | No |
...
## Code Like It's 198x curriculum

How far the teaching mission ([Code Like It's 198x](https://code198x.com)) reaches
each machine. Three tiers (from the Code Like It's 198x repo):
...
- **Not in Code Like It's 198x yet** — Sord M5, Memotech MTX, Tatung Einstein, SVI-328,
...
Cross-cutting views: [`../status/current-system-usability.md`](../status/current-system-usability.md)
(practical launch and usability status).
