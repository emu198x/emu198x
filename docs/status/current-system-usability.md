...
| Nintendo NES | `emu198x-nes`, `emu198x-script-nes` | Native verifier window with shared `wgpu` video and `raw`/`lcd`/`crt` modes plus headless cartridge runner with screenshots, live audio/audio capture, keyboard/gamepad controller input, scripts, and NROM/MMC1/UxROM/CNROM/MMC3/AxROM/BxROM/Camerica mapper support. | Smoke real Camerica/Codemasters software and add the next mapper needed by the compatibility matrix. |
