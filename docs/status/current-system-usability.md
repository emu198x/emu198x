...
| Dragon 32 | `emu198x-dragon`, `emu198x-script-dragon` | Early native verifier window with shared `wgpu` video, live PIA-derived mono audio pinned to XRoar's DAC/tape/single-bit level model, real Dragon 32 BASIC ROM boot, semantic keyboard input, CAS media mounting, native `--autoload` over ROM-level `CLOAD`/`CLOADM`, beam-updated MC6847 framebuffer, and optional patched-XRoar screenshot comparisons. | Validate sound-producing software, then add analogue joystick hardware. |
