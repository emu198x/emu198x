...
| Dragon 32 | `emu198x-dragon`, `emu198x-script-dragon` | Early native verifier window with shared `wgpu` video, live PIA-derived mono audio pinned to XRoar's DAC/tape/single-bit level model, real Dragon 32 BASIC ROM boot, semantic keyboard input, gamepad-to-analogue-joystick input, CAS media mounting, ROM/DGN cartridge mounting with GMC banking, PC-Dragon PAK snapshot mounting and snapshot smoke screenshots, native `--autoload` over ROM-level `CLOAD`/`CLOADM`, beam-updated MC6847 framebuffer, and optional patched-XRoar screenshot comparisons for CAS and PAK snapshot smokes. | Use PAK-vs-XRoar diffs to pin down remaining video state differences, then add true analogue host-axis input. |
