...
Blargg test compliance: 32/35 sub-tests pass across six suites. Four full
suites are clean (cpu_interrupts_v2 5/5, apu_test 8/8, instr_test_v5 1/1,
instr_misc 4/4). PPU VBL/NMI passes 9/10, MMC3 passes 4/6.

### Known gaps

Famicom Disk System support is not implemented, so broader compatibility still
depends on cartridge mapper coverage. The three remaining Blargg failures are
architectural edge cases: even/odd frame timing needs same-cycle $2001
visibility (tick loop limitation), MMC3 scanline timing is 1 PPU dot off, and
MMC6 IRQ behaviour is mutually exclusive with standard MMC3 rev B.
