...
33. **Shipping commits use conventional-commit prefixes.** `feat:` for new behaviour, `fix:` for corrected behaviour — these and only these bump the workspace version, because `release_commits` in `release-plz.toml` gates on exactly that pattern. Everything else (`docs:`, `chore:`, `refactor:`, `test:`) is fine and simply does not trigger a release. This implements [`knowledge/decisions/versioning-milestones.md`](knowledge/decisions/versioning-milestones.md), which is **Locked**: binary versions stay 0.x, driven by the release-plz minor/patch cadence on `feat`/`fix`. **Drift trigger:** if you are about to write a commit message that starts with a bare verb — "Add", "Fix", "Correct", "Constrain" — STOP and ask whether it ships behaviour. If it does, it needs a prefix. This rule is in RULES.md rather than only in the decision record because that is exactly how it was lost: **171 commits accumulated after `v0.2.0` with zero prefixes among them**, so release-plz prepared no release PR for a period that included a snapshot format bump. The convention was written down where nobody reads before committing.
