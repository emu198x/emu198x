...
_ZX80_DISPLAY_CODE = bytes([
    ...
    0x10, 0xFE, 0xD3, 0xFF, 0x21, 0x00, 0xC0, 0x06, 0x01, 0x0E, 0x2F, 0xCD,
...
# Assembled from `sinclair-zx81-display.asm` in this directory. To change the
# display routine, edit the assembly and regenerate:
#
#     asm198x asm --dialect pasmo --cpu z80 sinclair-zx81-display.asm -o out.bin
#
# then paste the bytes here. The rendering assertions in
# `synthetic_firmware_boot.rs` are what catch a bad paste — a wrong byte here
# shows up as a blank or ragged screen.
_ZX81_DISPLAY_CODE = bytes([
    ...
    ,
])


def sinclair_zx81() -> bytes:
    """ZX81 firmware that generates a picture the way the hardware does.

    ## What this proves

    The ZX81 has no frame buffer: the CPU executes through the display file
    while the ULA forces NOPs and turns the fetched bytes into pixels.
    Firmware that merely writes a display file and spins therefore renders
    **nothing at all** — which is what this image used to do, and why its
    test could pass against an emulator with no display generation in it.

    This one runs a real display routine, driven by the ULA's NMI line
    clock the way Sinclair's ROM is. See the assembly for the timing, which
    is measured rather than reasoned.

    The result is a solid 256x192 block: 24 rows of 32 glyphs, each `$80` —
    glyph 0 inverted. The ROM is zero-filled, so glyph 0 is blank and
    inverting it sets every pixel. A glyph already solid in ROM would light
    the screen with the CPU switched off; this way the test depends on `I`
    being set, on the display file being written, *and* on the CPU executing
    it through the ULA.
    """
    rom = bytearray(b"\x00" * 0x2000)     # zero-filled: glyph 0 is blank
    rom[0:len(_ZX81_DISPLAY_CODE)] = _ZX81_DISPLAY_CODE
    return bytes(rom)


# Assembled from `sinclair-zx81-keyboard.asm` in this directory, the same way
# as the display image beside it.
_ZX81_KEYBOARD_CODE = bytes([
    0xC3, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    ,
])


def sinclair_zx81_keyboard() -> bytes:
    """ZX81 firmware that shows which keyboard row is being pressed.

    #1041 was the ZX81 ignoring every key, and it was downstream of #1032:
    nothing was wrong with the keyboard code, but the ROM never reached the
    state where it processes input. Only an end-to-end test could see that,
    and the one that exists needs Sinclair's ROM, which cannot be staged on a
    runner. This is the part that can.

    One character row per keyboard row: press anything in row N and character
    row N goes solid. Row *selection* is what that tests — a machine
    returning the same byte for every row would light the wrong band, or all
    of them, and a test that only counted ink would pass it.
    """
    rom = bytearray(b"\x00" * 0x2000)     # zero-filled: glyph 0 is blank
    rom[0:len(_ZX81_KEYBOARD_CODE)] = _ZX81_KEYBOARD_CODE
    return bytes(rom)
...
SINGLE_MACHINE = {
    ...
    "sinclair-zx81-keyboard.rom": sinclair_zx81_keyboard,
