...
"""Validate cross-field semantics in a Copper colour phase capture."""
...
def _validate_transition(
    ...
    transition = _object(transition_value, path)
    if transition.get("ordinal") != ordinal:
        raise CaptureValidationError(f"{path}.ordinal does not match sequence order")
    if transition.get("from_word") != from_word or transition.get("to_word") != to_word:
        raise CaptureValidationError(f"{path} colour words do not match the suite program")

    status = transition.get("status")
    sample = transition.get("sample")
    delta = transition.get("minus_marker_start_samples")
    if status == "observed":
        sample_value = _integer(sample, f"{path}.sample")
        if not 0 <= sample_value < capture_width:
            raise CaptureValidationError(f"{path}.sample is outside the source capture")
        expected_delta = sample_value - marker_start if marker_start is not None else None
        if delta != expected_delta:
            raise CaptureValidationError(
                f"{path}.minus_marker_start_samples does not equal measured endpoints"
            )
        return sample_value

    if status not in {"not-observed", "unmeasurable"}:
        raise CaptureValidationError(f"{path}.status is not recognised")
    if sample is not None or delta is not None:
        raise CaptureValidationError(
            f"{path} sample and delta must be null unless status is observed"
        )
    return None
...
def validate_capture_record(capture_value: Any, suite_value: Any) -> None:
    ...
    if _integer(machine.get("chip_ram_bytes"), "capture.machine.chip_ram_bytes") < _integer(
    ...
    geometry = _object(suite_case.get("geometry"), "suite case geometry")
    beam_mapping = _object(normalization.get("beam_mapping"), "normalization.beam_mapping")
    if beam_mapping.get("sample_beam_line") != geometry.get("sample_beam_line"):
        raise CaptureValidationError("beam mapping sample line does not match the suite")
    ...
    colors = _object(suite_case.get("color_program"), "suite case color program")
    guard_word = colors.get("guard_word")
    move_words = _array(colors.get("move_words"), "suite case move words")
    if len(move_words) != 4:
        raise CaptureValidationError("suite case must contain four move words")
    ...
    for index, observation_value in enumerate(observed_fields):
        ...
        marker_start = marker[0] if marker is not None else None
        transition_values = _array(observation.get("transitions"), f"{path}.transitions")
        if len(transition_values) != 4:
            raise CaptureValidationError(f"{path}.transitions must contain four records")
        transition_samples: list[int | None] = []
        previous_word = guard_word
        for ordinal, (transition_value, move_word) in enumerate(
            zip(transition_values, move_words)
        ):
            transition_samples.append(
                _validate_transition(
                    transition_value,
                    f"{path}.transitions[{ordinal}]",
                    width,
                    ordinal,
                    previous_word,
                    move_word,
            ...
            previous_word = move_word

        observed_samples = [sample for sample in transition_samples if sample is not None]
        if observed_samples != sorted(set(observed_samples)):
            raise CaptureValidationError(
                f"{path} observed transition samples must be strictly increasing"
            )
        expected_spacings = [
            right - left if left is not None and right is not None else None
            for left, right in zip(transition_samples, transition_samples[1:])
        ]
        if observation.get("adjacent_transition_deltas_samples") != expected_spacings:
            raise CaptureValidationError(
                f"{path}.adjacent_transition_deltas_samples does not equal measured edges"
            )
    ...
    if provenance.get("classification") not in {evidence_class, "diagnostic-only"}:
        raise CaptureValidationError(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate semantic relationships and bound files in a Copper "
            "colour output-phase capture record."
        )
    )
