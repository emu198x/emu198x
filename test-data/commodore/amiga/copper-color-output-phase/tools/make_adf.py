...
def pack_adf(boot: bytes, payload: bytes) -> tuple[bytes, int, int]:
    ...
    if len(boot) > BOOTBLOCK_BYTES:
        raise AdfError(f"boot block is {len(boot)} bytes; maximum is {BOOTBLOCK_BYTES}")
    if not payload:
...
def validate_adf(image: bytes, payload: bytes, sectors: int) -> None:
    ...
    if any(image[PAYLOAD_OFFSET + len(payload) : padded_end]):
...
def main() -> int:
    ...
    image, checksum, sectors = pack_adf(args.boot.read_bytes(), args.payload.read_bytes())
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_bytes(image)
    print(
        f"{args.output}: {len(image)} bytes, payload {len(args.payload.read_bytes())} "
        f"bytes/{sectors} sectors, boot checksum 0x{checksum:08x}, "
