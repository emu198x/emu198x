...
It binds the exact ADF and payload, verifies named files, requires ordered
adjacent fields, joins sample rows to observations, checks marker and colour
edge bounds, recomputes every relative measurement, and prevents producer
type from being promoted to a stronger evidence classification.
`--skip-file-checks` is for record development only.

Run all corpus-local regressions from the corpus root with:
...
- [`../schema/README.md`](../schema/README.md) defines interchange records.
