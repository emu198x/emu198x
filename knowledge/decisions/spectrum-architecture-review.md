...
**Closed 2026-06-28** (post-October, during the save-state rollout audit — the deferral above was deliberately lifted, not silently dropped). These variants *are* now wired through `runtime-sinclair-zx-spectrum` (the runtime gained `pentagon_128`, `scorpion_zs256`, `timex_tc2048`, `timex_ts2068` modules since this doc was written), so the fallback became reachable. `PentagonUla::reattach_config` (→ `CONFIG_PENTAGON`), `ScorpionUla::reattach_config` (→ `CONFIG_48K`, a structural mirror), and `TimexScld::reattach_config(config)` (the SCLD is shared, so the caller passes `CONFIG_48K` for TC2048/TC2068 or `CONFIG_TS2068` for the NTSC TS2068) all landed; the four cores gained `restore_volatile_refs`, and the runtime's `after_restore` now calls it. Only the `&'static UlaConfig` was lost on restore — the hi-res framebuffer width is a serialised field and already survived. Regression tests `pentagon_restore_keeps_pentagon_ula_timing` and `ts2068_restore_keeps_ntsc_ula_timing` assert INT fires at the variant's own scanline after restore (Pentagon 256, TS2068 224), which the 48K fallback (248) would fail.
