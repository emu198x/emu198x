...
| 2026-05-18 | **Root cause identified: post-load keypress sequence missing from catalogue autoload, not an emulator bug.** Steve loaded Rainbow Islands in FUSE: music plays on the title screen, but only after the "stop tape and press any key" screen, and only on first load. Reproduced in our emulator by loading the SkoolKit `.z80` snapshot via `--headless --script` and injecting a sequence of brief keypresses (ENTER + 0 + Y, each tapped over ~10 frames with ~50-frame gaps). Result: 7,068 AY register writes across the capture window, 6,945 unique sample values, peak full-rail audio — **real music playing**, plus the screenshot advances past the static attract title to actual gameplay (in-game castle scene with player character visible). Verified the failure mode: single keypresses (just ENTER, just 0, just Y, or three taps of ENTER) all leave the game at 16 AY writes (init only). The game has **multiple chained press-any-key prompts** that need different keys to advance — probably "press any key to start" → "select controls / pick keyboard layout" → "press FIRE to begin" or similar — and the catalogue's autoload-then-capture flow never satisfies any of them. So the silence isn't a Z80 bug, isn't a Speedlock decryption bug, isn't an AY mixing bug — it's that **our captured-audio waypoint sits in the pre-music post-load wait state, where the music driver's IRQ is genuinely disabled by the game's own DI** because the game expects user input before starting its main loop. **The fix is in the catalogue, not the emulator**: per-entry post-load key sequences expressed as `input` steps in the manifest's `script[]` array. The catalogue runner already executes `script[]` steps between media load and the boot waypoint, and the script vocabulary already includes `input` for keypress events — no new infrastructure needed, just per-entry research into which key combo each title needs and then editing the manifest. IFF1 stays 0 across 200s of execution (`rainbow_islands_iff_trace.rs` confirms) — but that's CORRECT: real hardware does the same DI until the press-any-key is satisfied. |
