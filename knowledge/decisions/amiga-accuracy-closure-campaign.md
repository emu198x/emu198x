...
| 2026-08-01 | 4. Workbench 1.3 baseline audit | The six-row boot-golden matrix passes five rows. The A500+A501 Workbench 1.3 row remains at the AmigaDOS startup window instead of the committed desktop at frame 2,500. Clean revision `85d3dd05` reproduces the same state at frames 2,500 and 3,000, so the Lisa and full-raster corrections did not introduce it. Live inspection finds a Workbench process while the Initial CLI remains waiting, consistent with an unresolved startup/`LoadWB` handoff. The host-time-backed A501 RTC is also a separate deterministic-execution gap. The golden must not be replaced until both conditions are classified. |
