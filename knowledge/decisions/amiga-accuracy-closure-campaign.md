...
- The A500+A501 OCS PAL Test Kit v1.21 lane compares six stable video cases
  ...
  Four cases agree exactly. Gradients and EBU bars retain exact registered
  `denise-ocs-color-output-phase` disagreement signatures. This is one
  independent implementation family, not hardware evidence, and does not
  generalise to ECS, AGA, NTSC, audio or arbitrary software.
...
- the declared OCS exact cases and registered disagreement signatures remain
  green;
- the claimed AGA exact cases and registered pointer-disagreement signatures
  remain green against the independent producer;
...
| 2026-07-31 | 3. Registered Paula comparison | The reproducible vAmiga adapter and an audited three-case package now retain vAmiga 4.4b12 revision `60fd1e6b69dcd77c9f44d1291bd37ec715362ab0`, source WAVs, configurations, logs, hashes, and semantic records. Emu198x agrees on output assignment, programmed cadence within approximately 0.00486 percent under the three-field zero-crossing estimate, and the half/full volume relationship. This satisfies the Paula portion at the single-family software-evidence level; it does not establish hardware or two-family consensus. |
| 2026-08-01 | 3. Registered A1200 visual comparison | A reproducible FS-UAE 5.0.7 package supplied seven images for six A1200 AGA PAL Test Kit v1.21 cases. Under the then-used content-derived crop, the strict lane agreed on every RGB8 pixel and exposed Lisa's one-hires-pixel `COLORxx` output delay plus a board wrapper that paused Denise's bitplane pipeline before the retained viewport. The beam-absolute reconciliation recorded on 2026-08-08 supersedes that alignment claim and retains the remaining pointer and OCS colour-phase disagreements explicitly. |
...
| 2026-08-08 | 3. Beam-absolute Test Kit reconciliation | The A1200 producer and runtime viewports now use the fixed `raw + 8` PAL beam mapping. The earlier content-derived `+6` mapping hid a Lisa bitplane phase. Lisa now adds one lores bitplane/DIW output stage without moving OCS/ECS or sprite coordinates; AGA EBU bars, dots and crosshatch are exact, and every remaining AGA difference is confined to the pointer. Restoring the OCS/ECS phases returns the four non-Copper A500 cases to exact agreement. A500 gradients/EBU and the A1200 pointer are retained as exact registered disagreement signatures pending stronger evidence. The programmable-HBLANK write-timing grid remains exact in all ten cases. |
| 2026-08-08 | 4. A1200 Workbench baseline requalification | The corrected Lisa bitplane/DIW phase changed 14,550 of 430,144 pixels in the A1200 Workbench 3.1 boot golden. Translating the corrected image left by two host samples aligns every changed playfield pixel; the remaining 184 pixels occupy the pointer's 22 x 22 footprint because its absolute sprite coordinate did not move. The complete corrected image was reviewed and retained as an Emu198x regression baseline with no ignored region. The independent Test Kit contract, rather than this self-produced golden, remains the accuracy evidence. |
| 2026-08-08 | 4. Post-Lisa catalogue requalification | A complete ten-entry run at `456227ad` retained all ten snapshot/replay fixed points and eight existing frame/audio oracles. Only the two AGA frame baselines moved; both audio hashes remained exact. Workbench 3.1 follows the reviewed golden analysis above. The old Banshee x=0..765 region and corrected x=2..767 region agree at every one of 441,216 pixels, proving a two-host-sample playfield translation at the same settled POWERUPS waypoint. Both updated full-frame hashes then passed exact individual reruns. These remain Emu198x regression baselines; independent AGA accuracy evidence comes from the Test Kit contract. |
| 2026-08-13 | Clock, frame and same-cell ownership audit | The shared driver emits one chipset update per documented phase and exact stock, A1200 and rational A530 CPU-clock steps; no family-wide core overtick was found. The native UI did request four slices from a field-granular runtime and therefore advanced four fields per displayed frame; it now requests one. A separate post-Copper plan recomputation could allocate one physical cell to both Copper and a newly enabled bitplane; commit `61542515` freezes ownership before the MOVE. NTSC floppy index timing now derives from its own E-clock and remains at 300 RPM rather than the PAL-derived 302.75 RPM. Active 68020/68030 program prefetch remains conservatively word-shaped on 32-bit paths, an under-timing limitation rather than an overtick. |
