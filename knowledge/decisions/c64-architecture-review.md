...
**Status: corrected 2026-08-08.** The May audit was incomplete: it inspected only the SID source file and therefore missed seven skipped fields in the live VIC-II sprite fetch and draw pipeline. It also classified the SID output queues as disposable, although mixed samples are drained only at the next frame boundary and the diagnostic per-voice queues were not drained at all. A snapshot taken during an active sprite consequently lost the remainder of that sprite, and a mid-frame snapshot lost audio already generated for the next packet.
