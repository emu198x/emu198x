...
- [Speedlock silent music](decisions/speedlock-silent-music.md) — sibling of the tape thread; tape loads and title renders, but Speedlock's runtime anti-tamper silences AY music on Rainbow Islands / Bubble Bobble / Out Run / RoboCop / Op Wolf. Cracked Pentacle build plays music, original doesn't — proves it's the protection layer
- [Turrican / Tetris (+3) — closed](decisions/turrican-tetris-plus3-incomplete.md) — both closed 2026-05-12 via exec-phase read timeout (Turrican) + rotational `ReadID` (Tetris); catalogue entries land alongside
...
- [Amiga full-family architecture review](decisions/amiga-full-family-architecture-review.md) — preparing the seams for OCS / ECS / AGA / CDTV / CD32 / Vampire; Seam 1 (shared chip substrate) landed; Seams 2–5 sequence the rest
- [Amiga machine rollout plan](decisions/amiga-machine-rollout-plan.md) — order for building the remaining variants (A1200 → A600 → CDTV → A4000 → CD32 → A3000) and chip-extraction queue (Gayle, Alice + Lisa, Fat Gary + Ramsey, DMAC, Akiko, Buster)
