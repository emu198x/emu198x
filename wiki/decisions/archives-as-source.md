...
| `format-nintendo-nes-ines` (NROM only) | `Emu198x-archive/crates/format-nintendo-nes-ines/` | — | **Partially ported.** Emu198x commit `dd76911`. Mapper trait + header parser + NROM (mapper 0) only. 17 tests. 47 remaining mappers stay in archive. |
