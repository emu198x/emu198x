...
- **Datasette TAP software validation** — the fresh workspace now has ROM-backed real-title tape regressions for `Thinker` and `Thomas the Tank Engine`. `Thinker` reaches a second post-load `READY.` state, while `Thomas` reaches a stable `FOUND THOMAS` / `LOADING` / `READY.` screen under the real datasette flow. That is useful software pressure, but it is still not yet a full end-of-tape software-complete claim.
