...
- **D8. Extract `SpectrumMachineCore<M: MemoryBus>` to common.** Locked 2026-05-06; deferred for execution. The 48K-class machines (16K, 48K, Spectrum+) are electrically identical except for memory size. Rather than duplicating `Spectrum48k`'s 888-line `machine.rs` per variant or making `Spectrum48k` itself generic in the 48K crate (option B), the shared composition lives in `common-sinclair-zx-spectrum` as `SpectrumMachineCore<M: MemoryBus>` (option C from the architectural discussion). The 48K crate becomes `pub type Spectrum48k = SpectrumMachineCore<Spectrum48kMemory>` plus 48K-specific surface; 16K and Spectrum+ crates are similar thin wrappers. The 48K-class ULA (`FerrantiUla`) stays in the core; variants with different ULAs (128K family, Pentagon, Scorpion, Timex) keep their own machine implementations. Estimated effort: 4–6 hours (touch every method that references memory, ensure tests still pass at 90% coverage on the core). **D8 unblocks the 16K and Spectrum+ wrappers; +2 / +2A / +2B / +3 are not covered by D8 (different ULAs and memory paging).**
...
4. **Extract `SpectrumMachineCore<M: MemoryBus>` to common (D8).** The blocking refactor — 48K crate becomes a thin wrapper, then 16K and Spectrum+ wrappers fall out. ~4–6 hours. Next session.
5. Scaffold `machine-sinclair-zx-spectrum-16k` as a thin wrapper over `SpectrumMachineCore<Spectrum16kMemory>` (post-D8). Boot tests + ROM-region behaviour tests.
...
7. Scaffold `machine-sinclair-zx-spectrum-plus2`. Mirrors 128K's shape — own crate, reuses `sinclair-ula-7k010e`, adds +2-specific cassette and paging quirks. **Not covered by D8** (128K-family does not share `SpectrumMachineCore`).
8. D6 extraction: `-plus` (the existing crate handling +2A/+2B/+3 via Model enum) splits into `-plus2a`, `-plus2b`, `-plus3`. Each retains its current pattern (own machine implementation; not covered by D8 since they use the Amstrad gate array, not Ferranti).
