...
usage() {
    cat <<'USAGE'
Usage: scripts/verify-amiga-test-kit.sh

Runs the explicit Amiga Test Kit v1.12 accuracy gate for the stock A500
and the A500 with GVP A530 profile.

Environment:
    EMU198X_AMIGA_TEST_KIT_ADF       required Test Kit v1.12 ADF or ZIP
...
The input identities must match test-data/amiga-test-kit-v1.12.sha256.
Missing or mismatched inputs fail the gate.
USAGE
}
...
test_kit_source="${EMU198X_AMIGA_TEST_KIT_ADF:-}"
if [[ -z "${test_kit_source}" || ! -f "${test_kit_source}" ]]; then
    echo "error: set EMU198X_AMIGA_TEST_KIT_ADF to the Test Kit v1.12 ADF or ZIP" >&2
    exit 1
fi

if [[ -n "${EMU198X_AMIGA_KICKSTART_13_ROM:-}" ]]; then
...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/emu198x-amiga-test-kit.XXXXXX")"
trap 'rm -rf "${work_dir}"' EXIT

normalized_adf="${work_dir}/amiga-test-kit-v1.12.adf"
normalized_rom="${work_dir}/kickstart-1.3-r34.5.rom"
...
if command -v sha256sum >/dev/null 2>&1; then
    (
        cd "${work_dir}"
        sha256sum -c "${repo_root}/test-data/amiga-test-kit-v1.12.sha256"
    )
else
    (
        cd "${work_dir}"
        shasum -a 256 -c "${repo_root}/test-data/amiga-test-kit-v1.12.sha256"
    )
fi

export EMU198X_AMIGA_TEST_KIT_ADF="${normalized_adf}"
export EMU198X_AMIGA_KICKSTART_13_ROM="${normalized_rom}"
