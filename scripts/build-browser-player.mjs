#!/usr/bin/env node
// Shared, firmware-free distribution consumed by both public sites.
...
try {
  ...
  if(process.env.WASM_BINDGEN)execFileSync('cargo',['build','--locked','--release','--target','wasm32-unknown-unknown',...families.flatMap(f=>['-p',`emu198x-${f}-web`])],{cwd:root,stdio:'inherit'});
  for(const family of families) {
    ...
    } else execFileSync('wasm-pack',['build',path.join(root,'crates',crate),'--target','web','--release','--out-dir',out,'--no-pack','--no-opt'],{cwd:root,stdio:'inherit'});
  ...
  const catalogue=makeCatalogue(profiles);
  ...
  writeFileSync(path.join(stage,'build.json'),JSON.stringify({revision,modified,families,fleetFamilies},null,2)+'\n');
