...
usage() {
    cat <<'USAGE'
Usage: scripts/verify-amiga-test-kit-video.sh

Runs the explicit Amiga Test Kit v1.21 video-reference accuracy gate
for the PAL A500+A501 OCS profile.
...
The input identities must match test-data/amiga-test-kit-v1.21.sha256.
...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/emu198x-amiga-test-kit-video.XXXXXX")"
...
if command -v sha256sum >/dev/null 2>&1; then
    (
        cd "${work_dir}"
        sha256sum -c "${repo_root}/test-data/amiga-test-kit-v1.21.sha256"
    )
else
    (
        cd "${work_dir}"
        shasum -a 256 -c "${repo_root}/test-data/amiga-test-kit-v1.21.sha256"
    )
fi
